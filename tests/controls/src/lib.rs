...
//! Shared helpers for per-control property tests.
//!
//! For a given control (`ControlType`) the harness:
...
//!
//! A property "works" here = it is defined on the control, settable, and
...
/// Run the full property check for one control. Returns one `PropResult` per
/// (property × stage).
pub fn check_control(ct: ControlType, label: &str) -> Vec<PropResult> {
    ...
    let mut form = Form::new("TEST-FORM", "control Test", 800, 600);
    form.controls.push(ctrl);

    let path = std::env::temp_dir().join(format!("rcobol_controltest_{label}.cfrm"));
...
pub fn assert_control(ct: ControlType, label: &str) {
    let results = check_control(ct, label);
    let total = results.len();
    let mut failures = 0;

    println!("\n=== control '{label}' — {total} property checks ===");
    for r in &results {
    ...
    assert_eq!(
        failures, 0,
        "control '{label}' had {failures}/{total} failing property checks"
    );
}
