...
: GroupBox, Panel, TabControl, Splitter, MenuBar, ToolBar, StatusBar.
  ...
  **Splitter は 2 つに分割されたパネル**であり、上記 3 つと同じくコンテナです。
  ...
  - **線が動いたときに中身がどうなるか**は、ペインごとに選べます。Splitter ではなく
    ペインのプロパティ **Pane Left/Right Resize Behavior** で設定します。

    | 動作 | そのペイン内のコントロールの挙動 |
    |---|---|
    | **Translate with divider**（既定） | 各コントロールは分割線からの距離を保ちます。両方のペインでそうなるので、線を右へ 40pt ドラッグすると両側の中身がすべて右へ 40pt 動きます。コントロールはペインの反対側の端を越えて運ばれることがあり、そこで切り取られます。 |
    | **Scale within the pane** | 各コントロールはペインに対する*割合*で位置を保ちます。ペインを広げれば中身が広がり、狭めれば詰まります。サイズは決して拡大縮小されず、位置だけが変わるので、何も歪まず、ペインの外へも出ません。 |
    | **Anchor to the outer edge** | 各コントロールは自分のペインの先頭側の端からの距離を保ちます。ペイン 1 の先頭側の端は Splitter 自身の端で動かないため中身はそのまま、ペイン 2 の先頭側の端は分割線そのものなので中身は線と一緒に動きます。通常のコンテナと同じ挙動です。 |

    2 つのペインは独立しています。片側に固定のコントロール列、もう片側に伸縮する
    領域、という構成は、一方を *Anchor*、他方を *Scale* にするだけです。

    デザイナーで分割線をドラッグすると、**コントロールは実際に移動します**。X/Y が
    書き換えられて保存され、そのドラッグ全体（線と、運ばれたものすべて）が
    1 回の取り消しになります。
