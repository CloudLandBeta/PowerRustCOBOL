...
> **注意 — Neumorphic。** 该样式使用与整个窗体相同的阴影堆栈绘制自己的柔和浮雕
> （光源在左上方），因此 `Fixed3D`、`Raised` 和 `Single` 在它下面都表现为凸起。
> **`Sunken` 会把这层浮雕翻转过来**——阴影移到上边和左边、高光移到下边和右边——
> 于是控件看起来像被按进窗体里。这就是 `BorderStyle` 在 Neumorphic 下的全部
> 含义：凸起，或者按下。
