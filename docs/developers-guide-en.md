...
| `settings-form` | The IDE's providers in a ComboBox (`COBOL-PROVIDER-COUNT/GET`), the provider's models listed with `COBOL-MODEL-LIST`, a **Test connection** button (`COBOL-MODEL-TEST`); a model list the program keeps in its own indexed file, handed over with `COBOL-MODEL-SET`, and keys stored with `COBOL-KEY-SET` — never shown again; the KB folder picked with `COBOL-FOLDER-DIALOG`; the whole setup exported to and imported from XML through the save and open dialogs — without the keys; a tooltip on every field |
