...
RustCOBOL implementa realmente — e para demonstrá-lo contra a **suíte oficial de
validação NIST COBOL‑85** em vez de simplesmente afirmá-lo. O
...
COBOL 85 versão 4.2, SSVG de abril de 1993), a suíte que o National Institute of
...
O livro de registro legível por máquina é
...
|---|---:|---|
| **Compilação** | **420 / 420** | o front end aceita todos os programas dentro do escopo. FAIL 0. |
...
A suíte pontua‑se a si mesma — cada programa CCVS85 imprime a sua própria
contagem `PASS` / `FAIL*` — de modo que o eixo de execução é o que significa
...
programas de arquivos RELATIVE compilavam limpos enquanto o runtime **não tinha
...
de modo que apenas os programas chamadores são pontuados.
...
| **No escopo** | | **420 / 420** | **380 / 380** | **8.362** | |
| SG | Segmentação | 13 / 13 | — | — | ⬜ declarado fora do escopo (abaixo) |
...
o eixo de execução para ele não foi declarado dentro do escopo. É listado aqui em
...
programa saltou por si mesmo. **Não** é uma passagem, e é registrado em separado
...
comentados, de modo que uma execução conforme do fonte entregue reporta exatamente
esses três. Os outros 21 estão registrados mas ainda não explicados
individualmente no livro de registro; não os cites como intencionais.
...
|---|---:|---|
| **CM** — Comunicação | 9 | `COMMUNICATION SECTION`, entradas `CD`, `SEND` / `RECEIVE` / `ENABLE` / `DISABLE`. Visa os monitores de teleprocessamento dos anos oitenta — filas de mensagens na posse de um gerenciador de transações. Não existe aqui tal runtime, e o módulo foi retirado dos padrões COBOL posteriores. |
| **RW** — Report Writer | 6 | `REPORT SECTION`, entradas `RD`, `INITIATE` / `GENERATE` / `TERMINATE`, quebras de controle. Uma sublinguagem declarativa extensa; a resposta do PowerRustCOBOL aos relatórios é o Form Designer e a exportação para PDF. Poderia tornar‑se mais tarde uma *funcionalidade* caso se queira — é a única exclusão com valor real para o usuário. |
| **SG** — Segmentação | 13 | Decisão do operador, 2026‑08‑29. A segmentação existe para encaixar um programa numa máquina pequena demais para contê-lo: os cabeçalhos `SECTION` levam um número de segmento e o runtime sobrepõe segmentos independentes uns aos outros. O RustCOBOL é um runtime de 64 bits com mais espaço de endereçamento do que qualquer programa COBOL consegue esgotar, de modo que um número de segmento **compila e não tem efeito nenhum**. Não há comportamento que o módulo possa medir. Os seus 13 programas continuam a compilar, e são reportados como N‑A em vez de eliminados para que a exclusão continue visível. |
| **OBSQ / OBIC / OBNC** | 9 | Voltam a testar módulos anteriores e esperam que o compilador *sinalize* elementos obsoletos do COBOL‑85. O seu conteúdo de linguagem está coberto pelas especificações dentro do escopo; o que está fora do escopo é a **sinalização** de funcionalidades obsoletas. |
| **EXEC85** | 1 | Não é um teste. É o próprio executivo COBOL do NIST que divide a distribuição e comanda a suíte — aqui substituído por um arnês em Rust, de modo que não precisa compilar. |

O **COBOL orientado a objetos** também está fora do escopo do RustCOBOL, mas o
CCVS85 é‑lhe inteiramente anterior — não há programas OO na suíte.
...
registrada acima com a sua razão, mais os 21 casos DELETED que não foram
...
> uma só vez. Qualquer contagem bruta de marcadores retirada do arquivo de
> impressão tem de dividir as falhas por dois antes de significar algo.

### História de conformidade

Eixo de compilação, contra o denominador dentro do escopo de cada momento. O
próprio denominador moveu‑se quando o SG foi declarado fora do escopo e quando o
DB205A foi repontuado sob o CM, de modo que as primeiras linhas são sobre 434 e a
...
| 1.62.16 | 376 / 434 | O `AT` de `AT END` é opcional, de modo que uma frase `END` solta já não engole o cabeçalho de parágrafo seguinte (33 programas). **E/S indexada 42 / 42 na compilação.** |
...
| **1.62.43** | 422 / 434 | A E/S sequencial compila por completo, 85 / 85, e vai de 10 → 44 de 85 na execução. Os parágrafos de um declarativo conservam os nomes, de modo que um tratador `USE` lhes pode fazer `PERFORM` e `GO TO` — 20 programas deixaram de rebentar. |
| **1.62.47** | — | **E/S sequencial terminada** — 85 / 85 em ambos os eixos. A última lacuna era `XXXXD001`, um arquivo de dados que a *instalação* do CCVS85 fornece e que nenhum membro escreve; agora o arnês planta‑o. |
| **1.62.76** | — | Chega o **motor RELATIVE** (`cobolt-runtime/src/relative.rs`, contentor `PRCREL1`). Os sete verbos de arquivo despacham sobre `FileOrganization::Relative`. |
| **1.62.77** | — | **E/S relativa terminada** (34 / 34) desde uma linha de base de 14 / 35 numa única sessão, e **E/S indexada terminada** — o motor relativo fechou as últimas quatro falhas do IX106A, que eram o arquivo relativo que ele exercita ao lado dos sequenciais e indexados. |
...
| **1.62.129** | **420 / 420** | **O censo de compilação fecha a 100 %.** O DB205A é pontuado sob o CM por decisão, o que coloca a suíte dentro do escopo em 420. |

> **O resumo honesto.** Todos os programas dentro do escopo compilam, e todos os
...
> **nomes‑condição de nível 88 reais** (`SET … TO TRUE/FALSE`, o host é
...
> **`ACCEPT`/`DISPLAY`** de tela estendidos (`AT`/`WITH` através de ANSI em modo
> CLI — agora *executados*, e não apenas analisados).
>
> **Atualização (1.7.1):** as fontes de registro do `ACCEPT` são agora funcionais
...
> **`CRT STATUS`** → `"0000"`.
>
> **Atualização (1.7.2):** frases de partilha / bloqueio de arquivos e `CANCEL`
> (eram ❌ / no‑ops) — **`OPEN … SHARING WITH … [WITH LOCK]`**,
> **`READ … WITH [NO] LOCK`**, **`UNLOCK`** (liberta os bloqueios de registro
> INDEXED do arquivo), e **`CANCEL program`** (reinicializa o armazenamento do
> programa).
>
> **Atualização (1.8.0):** **`COMMIT` / `ROLLBACK`** são agora verbos COBOL reais
> — transações controladas pelo programa sobre os arquivos INDEXED abertos (tanto
> o motor de memória como o de disco). O motor de disco ganhou um registro de
...
- ✅ Os parágrafos de **entrada‑comentário** — `AUTHOR`, `INSTALLATION`,
  ...
  subconjunto**.
- ✅ `REMARKS` também é aceite. Foi eliminado do COBOL em 1985, de modo que não é
...
DIVISION, de modo que um dado chamado `SECURITY` continua a funcionar.
...
adivinhada a partir do conteúdo do arquivo, porque aplicar regras de colunas a
fonte que não foi escrito para elas apaga código em silêncio.

| `--source-format` | O que significa |
|---|---|
| `free` | Sem regras de colunas nenhumas. `*>` inicia um comentário. **O valor por padrão**, e o que os próprios projetos do PowerRustCOBOL e os arquivos `.cbl` de formulário gerados usam. |
...
`COBOLT_SOURCE_FORMAT` define o valor por padrão de uma sessão.
...
- **Coluna 7** — área indicadora:
  ...
  - qualquer outra coisa → lida como fonte comum. O padrão reserva esta coluna,
    mas as suítes de imagem de cartão usam‑na como seletor de linhas opcionais, e
...
*próxima* aspa em qualquer ponto do arquivo, de modo que um único `"` perdido num
...
**par** de aspas, de modo que nada ficava por terminar; um único carácter havia
deslocado a paridade de todo o arquivo. Agora o dano para na mudança de linha.
...
> **Nota.** Escolher `fixed` para um arquivo que foi escrito em formato livre
...
`UNLOCK file` (liberta os bloqueios de registro do arquivo) ·
`OPEN … SHARING/WITH LOCK` · `READ … WITH [NO] LOCK` (partilha/bloqueio de
arquivos — indicativo dentro de uma única unidade de execução)
✅ `COMMIT` / `ROLLBACK` (transações de arquivos INDEXED controladas pelo
programa — ver os verbos de arquivo) · `CANCEL` (reinicializa o armazenamento do
programa) ·
✅ `INVOKE` — comanda objetos de GUI/runtime (janelas, formulários, métodos de
controle); é um no‑op apenas para os objetos **COBOL**, já que as definições de
classe/método estão fora do escopo
...
Crates do Projeto** (spec 044): os crates registrados são fixados numa versão
exata, copiados para o `crates/` do projeto e compilados dentro do binário; os
crates não registrados fazem falhar o Check/Build na linha do desenvolvedor,
nomeando o remédio.
...
conservam os nomes, de modo que lhes pode fazer `PERFORM` e `GO TO` — incluindo um
parágrafo de *outra* secção declarativa. Os parágrafos declarativos vivem no seu
próprio espaço de nomes: o controle nunca cai do corpo principal para dentro
deles, e um nome declarado em ambos resolve para a cópia do declarativo enquanto
um tratador está rodando e para a do corpo em todo o resto. Um declarativo
...
- ✅ **Um operando de grupo torna todo o movimento alfanumérico** (COBOL-85
  ...
  grupo: cada filho toma a sua fatia literalmente, de modo que um filho
...
- ✅ **Um par precisa que apenas UM dos seus dois itens seja elementar.** Um grupo
  ...
  não possui espaço de armazenamento, de modo que a escrita ia para onde nada a
...
- ✅ **Modificação de referência `id(start:len)`** — emissor (subcadeia) e recetor
  ...
  `length` é opcional. Endereça **posições de carácter**, de modo que um operando
...
- ✅ **`WITH TEST AFTER` aplica‑se a `VARYING`**, escrito de qualquer dos lados da
  ...
  o seu teste sai falso, de modo que o teste que termina o ciclo deixa‑a como o corpo
  a deixou.
...
- ✅ **Um identificador `VARYING` com subscrito segue o seu subscrito.**
  ...
  a ocorrência que `S1` selecionar nesse momento, de modo que um corpo que avança `S1`
  percorre a tabela.
...
- ✅ **O qualificador `{OF|IN} section` escolhe qual cópia se pretende** quando um
  ...
  1.62.39 o qualificador era analisado e depois ignorado, de modo que o salto aterrava
...
- ✅ `NEXT SENTENCE` — transfere o controle para além do limite de frase seguinte
...
- ✅ **`FROM mnemonic-name` lê do operador** quando `SPECIAL-NAMES` declara o
  ...
  cláusula comum `<implementor-name> IS <mnemonic>` era saltada por completo, de modo que todos os mnemónicos liam uma variável de ambiente que nunca era definida e o
  item recetor ficava vazio.)*
- ✅ `ACCEPT id AT {nnnn | LINE n COLUMN n}` posiciona o cursor (ANSI, CLI).
- ✅ `FROM COMMAND-LINE` (toda a linha de comando) · `FROM ARGUMENT-NUMBER`
...
- ✅ `END-DISPLAY` fecha a lista de operandos (opcional), de modo que
...
- ✅ **O resultado é montado byte a byte**, de modo que `STRING HIGH-VALUE` move o
  único byte `0xFF` e ocupa uma posição de carácter.
- ✅ **Extensão — `DELIMITED BY` inteligente por padrão** (quando nenhuma frase
  governa um operando): os itens alfanuméricos `PIC X`/`A` assumem `SPACES` por
  padrão (o enchimento final é descartado); os literais de cadeia, os itens
...
- ✅ **Uma série de operandos TALLYING partilha UMA ÚNICA passagem da esquerda
  ...
  corresponder desde a extremidade esquerda da sua janela sem intervalo, de modo que
...
- ✅ **Uma série de operandos REPLACING partilha UMA ÚNICA passagem também**, pela
  ...
  caracteres e a passagem retoma para além deles, de modo que nenhum operando
...
- ✅ **Um item DISPLAY com sinal não tem um `-` entre as suas posições de
  carácter.** O sinal operacional é uma sobreperfuração num dígito, de modo que
  `INSPECT <PIC S9(5) que contém -12345> TALLYING c FOR ALL "-"` dá **0** enquanto
  `FOR ALL "5"` dá 1. O sinal é restaurado depois, de modo que um `REPLACING` sobre os
...
- ✅ `SET 88-name TO TRUE` define o item host para o primeiro VALUE da
...
- ✅ **Um só `OPEN` pode levar vários grupos de modo**, cada um com os seus próprios
  arquivos: `OPEN INPUT SQ-FS1, SQ-FS3 OUTPUT SQ-FS4.` Cada grupo é aberto no seu
  próprio modo; `SHARING` / `WITH LOCK` / `REGISTERED USER` aplicam‑se à instrução.
- ✅ **Um `OPEN` de um arquivo que já está aberto é `41`**, e o arquivo é deixado
  como estava — a instrução **não** o reabre. (Reabrir um arquivo `OUTPUT`
  truncaria em silêncio o que o programa já tivesse escrito.)
- ✅ **`OPEN … WITH REGISTERED [USER] {literal | data-item}`** (extensão do
  PowerRustCOBOL) — registra o operador/usuário no registro de observabilidade do
  INDEXED (campo `user=` em cada linha de evento da sessão desse arquivo).
...
- ✅ `READ f [RECORD] [{NEXT|PREVIOUS}] [INTO id] [KEY IS k] [WITH [NO] LOCK]
  ...
  `WITH NO LOCK` liberta o bloqueio de registro que o motor INDEXED toma sob I‑O.
- ✅ **`READ … INTO id` é o `READ` seguido de um `MOVE` de grupo.** O registro é
  ...
  transporta bytes — um registro que contém um byte que não é um carácter chega
  intacto.
- ✅ **Cláusula `RECORD` da FD — registros de comprimento variável.** As três
  grafias: `RECORD CONTAINS n CHARACTERS` (fixo),
  `RECORD CONTAINS n TO m CHARACTERS` (variável; a descrição de registro que o
  ...
  limitado ao intervalo declarado). Uma FD cujos registros `01` diferem em tamanho é
  de comprimento variável, diga‑o ou não. Um arquivo de comprimento variável
  guarda o comprimento de cada registro junto ao registro, de modo que os seus bytes
  **não** são intercambiáveis com os de um arquivo de comprimento fixo; um
  arquivo de comprimento fixo não muda.
- ✅ **Os registros `01` de uma FD descrevem uma única área de registro.** Um `READ`
  entrega os bytes através de todas as descrições de registro; um `WRITE` envia a
  área completa, de modo que o que outra descrição de registro colocou onde a escrita
  tem `FILLER` transparece.
- ✅ **`FILLER` ocupa os seus bytes num registro de FD**, e
...
- ✅ **O `LINAGE` de uma FD aceita nomes de dados além de inteiros** —
  ...
  medida a partir desses itens em cada `WRITE`, de modo que um programa pode
  redimensioná‑la enquanto corre. `LINAGE-COUNTER` vale um quando o arquivo é
  aberto.
- ✅ **Um `READ` sequencial depois de `AT END` é `46`, e não um segundo `10`.** O
  `AT END` não deixou um registro seguinte válido, de modo que continuar a ler é um erro
  diferente de chegar ao fim. `46` é um estado de classe 4, de modo que nem `AT END`
  nem `NOT AT END` correm para ele — o declarativo `USE` do arquivo é o que o
  trata. Um `OPEN` novo, ou um `START` com êxito, volta a estabelecer um registro.
- ✅ `UNLOCK f [RECORD[S]]` liberta os bloqueios de registro do arquivo.
- ✅ **`COMMIT` / `ROLLBACK`** — transações controladas pelo programa sobre **todos**
  os arquivos INDEXED abertos. `OPEN` inicia uma transação; `COMMIT` confirma os
  ...
  (nunca escreve para disco); um arquivo `STORAGE IS MEMORY` simples é efémero, e
  `STORAGE IS MEMORY WITH PERSISTENCE` guarda para disco apenas no `CLOSE`. (A
  recuperação de falhas através de um registro de escrita antecipada durável é
...
- ✅ **`SELECT … STORAGE [MODE] IS MEMORY | DISK [WITH COMPRESSION] [WITH
  ...
  por padrão é `DISK`. `WITH COMPRESSION` comprime o registro armazenado (as chaves
  são avaliadas sobre o registro não comprimido); `WITH PERSISTENCE` (apenas MEMORY)
  guarda o arquivo em RAM no `CLOSE`. `OPEN OUTPUT` (re)cria sempre o contentor em
  disco.
...
- ✅ **`REWRITE` num arquivo SEQUENTIAL de registros** substitui o registro que o
  último `READ` entregou, no lugar, e deixa a posição de leitura onde estava — o
  `READ` seguinte continua a dar o registro que vem depois. Os estados que deve:
  **`49`** quando o arquivo não está aberto em `I-O`, **`43`** quando nenhum `READ`
  com êxito estabeleceu um registro (incluindo depois de `AT END`, e num segundo
  `REWRITE` sem `READ` de permeio), e **`44`** quando o novo registro não tem o mesmo
  comprimento do lido — num arquivo com `DEPENDING ON` o valor do item é esse
...
- ⚠️ A partilha de arquivos entre *processos* não é imposta (uma única unidade de
  execução); as frases `SHARING`/`LOCK` são analisadas e os bloqueios de registro por
...
- Os registros são ordenados de forma estável pelas chaves declaradas
  (`ASCENDING`/`DESCENDING`); `USING` lê / `GIVING` escreve os arquivos
...
- ✅ Classe: `id IS [NOT] {NUMERIC | ALPHABETIC | ALPHABETIC-LOWER | ALPHABETIC-UPPER}`.
  ...
  analisava os caracteres como um número, de modo que um sinal, um ponto decimal, um
  expoente e os espaços em volta eram todos aceites.)*
- ✅ **Um operando de `CLASS` definido pelo usuário pode ser uma posição
...
- ✅ **`TRUE` / `FALSE` como operandos** (extensão do PowerRustCOBOL) — açúcar para
  ...
  `SET <88‑name> TO TRUE` continua a definir o item host para um valor que
...
- ✅ **Um nome‑condição declarado num grupo testa os bytes do grupo.** Um grupo não
  possui armazenamento próprio — *é* os seus filhos — de modo que
  `01 T. 88 B VALUE "ABCABC". 02 A PIC XXX. 02 B2 PIC XXX.` compara contra os seis
  caracteres que o registro contém.
- ✅ **Uma constante figurativa é repetida até ao tamanho do outro operando**, e
  isso inclui uma escrita como o `VALUE` de um 88: `88 B VALUE QUOTE` num host
...
- ✅ **Os registros de data e hora leem o relógio LOCAL.** `ACCEPT … FROM DATE /
  ...
  em relação a GMT (`…-0300`), de modo que um programa consegue saber em que fuso está rodando.
...
- ✅ **Uma constante figurativa enche todo o seu recetor**, incluindo
  ...
  colocar os seus caracteres de inserção, de modo que um `PIC XX0XXBXXX` contém
...
- ✅ **Um literal numérico pode começar pelo ponto decimal** — `.5`, `-.5`,
  `.000000001`. O COBOL‑85 exige apenas que um literal não *termine* num, de modo que
  ...
  o COBOL‑85 exige um depois de um terminador, de modo que `MOVE X TO Y.` nunca é lido
...
- ✅ **`SPECIAL-NAMES. CURRENCY [SIGN] [IS] literal`** — o carácter que enche uma
  ...
  ele, de modo que assim que um programa declara um, `$` já não é um carácter de
  picture ali:
  ```cobol
  SPECIAL-NAMES.
      CURRENCY "<".
  ...
  cinco posições, de modo que `MOVE 12` se lê como `  W12`. *(Antes de 1.62.40 uma
  série de um símbolo de letra era lida como uma só palavra e rejeitada, de modo que
...
- ✅ **Literais hexadecimais** — `X"09"`, `x'0D0A'` (em qualquer caixa, com
  qualquer aspa). Um carácter por **par** de dígitos hexadecimais, de modo que o número
...
- ✅ `PIC/PICTURE` com `X A 9 S V P` e símbolos de edição (`Z * $ + - CR DB B 0 /
  ...
  leem‑se sempre como zero e não ocupam **nenhum byte** na disposição de um registro.
- ✅ **A proteção com asteriscos enche todo o item.** Um valor zero numa picture
  ...
  de modo que o `$` fixo conserva a sua própria posição (`-2.34` → `$*2.34CR`).
  *(Antes de 1.62.37 `CR`/`DB` contribuía com um asterisco em vez das duas posições
  de carácter que ocupa, de modo que tal item voltava um carácter mais curto do que a
  sua própria largura.)*
- ✅ **Um literal numérico move os seus caracteres, tal como está escrito.** Para um
  ...
  de modo que um zero à esquerda era perdido e todos os caracteres seguintes se
...
- ✅ **Uma relação entre um operando numérico e um não numérico é não numérica**
  ...
  de ser um **inteiro**; «não numérico» é decidido pela **declaração**, de modo que um
  filho `PIC 99` que contém caracteres depois de um `MOVE` de grupo continua a ser
  numérico — e um **grupo** é não numérico sejam os seus filhos quais forem, de modo que
...
- ✅ **Truncamento pela esquerda num MOVE numérico.** Um recetor contém exatamente
  ...
  a capacidade do recetor, de modo que uma instrução com `ON SIZE ERROR` conserva em vez
...
- ✅ **Uma tabela de grupos é endereçada por ocorrência.** `MOVE VALUES-1 TO
  ...
  registro `01` que os engloba são os bytes de **todas** as ocorrências, de modo que
...
- ✅ `REDEFINES` — uma segunda leitura **viva** dos mesmos bytes. Não acrescenta
  armazenamento (de modo que não alarga o grupo que o contém), e uma escrita através de
...
- ✅ **As sobreposições aninham‑se.** Um `REDEFINES` dentro de um registro que por sua
  vez está redefinido é alcançado em ambas as direções, por profundo que seja:
  escrever dois bytes através de uma redefinição de nível 01 alcança o registro
...
- ✅ **Uma descrição sem nome continua a ser uma descrição.**
  ...
  nenhuma, de modo que os seus filhos liam‑se como espaços por muito que o alvo tivesse
  sido enchido.)*
- ✅ **Um nome duplicado dentro de uma sobreposição** resolve para o mesmo
  ...
  qualificadores exteriores, o que só um nome duplicado consegue distinguir — de modo que precisamente o caso que precisa do qualificador perdia‑o.)*
- ✅ `JUSTIFIED [RIGHT]` — **armazena alinhado à direita**, num item *alfanumérico*
  ...
  cláusula era registrada apenas para itens alfanuméricos, de modo que
...
- ✅ **Um `REDEFINES` no nível 01 pode descrever mais armazenamento do que o item que
  ...
  longa o suficiente para nomeá-los. Escrever através de uma descrição mais curta
...
- ✅ **Uma sobreposição `REDEFINES` transporta os bytes do item redefinido**,
  ...
  **não** para eles. Quando os bytes formam de fato dígitos a leitura numérica não
  muda.
- ✅ `88 name VALUE v [v …]` / `VALUE a THRU b` — **nomes‑condição reais**: o nível
  88 liga‑se ao seu item host; o teste verifica o host contra os VALUE /
  intervalos, e `SET 88-name TO TRUE` guarda um valor que os satisfaz no host.
- ✅ **Um nome‑condição pode ser declarado sob mais do que um grupo, e `OF`/`IN`
  ...
  ```
  O subscrito pertence ao item host, de modo que seleciona contra que ocorrência os
...
- ✅ `USAGE INDEX` declara um registro de índice inteiro (`SET`/`SEARCH` usam‑no);
...
- ✅ `66 NEW RENAMES item-1 [{THRU|THROUGH} item-2]` — um alias de reagrupamento; a
  ...
  - ✅ **Um 66 é qualificado pelo registro que reagrupa**, exatamente como um dado é
    qualificado pelo grupo acima dele, de modo que o mesmo nome de 66 pode ser declarado
    uma vez por registro e distinguido com `OF`/`IN`:
    ...
    o seu nome. Os operandos da cláusula `RENAMES` resolvem nesse mesmo registro, de modo que um `NAME-2` duplicado nomeia o deste registro.
  ...
  - ✅ **Um 66 sobre exatamente um item *é* esse item** — mesma PICTURE, mesma
    ...
    numérico de quatro dígitos, de modo que `ADD 3500 TO R` com 8000 dentro provoca
...
> / cláusulas do COBOL‑85 está **coberto por completo**». Executar a suíte NIST
> CCVS85 desmentiu‑o: **102 dos 434 programas dentro do escopo falharam naquele
...
> ponto decimal inicial. É para isso que serve uma suíte de validação. Cada lacuna
...
A lista abaixo é o que está fora do escopo **por intenção**, em contraste com as
lacunas do NIST acima, que são defeitos em vias de resolução:

1. **Edição de entrada no `ACCEPT` de tela** — `DISPLAY … AT/WITH` e `ACCEPT … AT`
...
2. **Partilha de arquivos entre *processos*** — `OPEN … SHARING/WITH LOCK`,
   `READ … WITH [NO] LOCK` e `UNLOCK` são analisados e comandam os bloqueios de
   registro por execução do motor INDEXED, mas os bloqueios não são impostos entre
   processos distintos do sistema operacional (modelo de uma única unidade de
   execução).
...
4. ✅ **Resolvido (1.62.15).** Um nome de função intrínseca não reconhecido devolvia
   **0** em silêncio, de modo que um programa calculava com toda a confiança uma
...
5. ⚠️ **Um valor inválido de `ACCESS MODE` / `ORGANIZATION` é engolido sem
   ...
   escrita comum do usuário. `ACCESS MODE IS` aceita apenas `SEQUENTIAL`,
   ...
   cair no ramo genérico de «saltar um token desconhecido», de modo que o arquivo
   conserva em silêncio o `SEQUENTIAL` por padrão e comporta‑se mal em tempo de
   ...
   suíte escreve apenas cláusulas válidas, de modo que todos os módulos podem terminar
   a 100 % com a lacuna ainda aberta. É uma armadilha de erro de escrita do
   usuário, e precisa de um teste próprio em vez de uma pontuação de módulo.
6. ⚠️ **`ALPHABET … IS EBCDIC` é aceite mas deixa em vigor a ordenação nativa
   (ASCII).** A frase literal (`"A" THRU "H" "I" ALSO "J" …`), `NATIVE`,
   `STANDARD‑1` e `STANDARD‑2` estão todas implementadas e comandam de fato
...
> `NEXT SENTENCE` fiel; as intrínsecas padrão restantes; e `ACCEPT`/`DISPLAY` de tela
> estendidos (executados em modo CLI).
> **Resolvido (1.7.1):** `ACCEPT FROM COMMAND-LINE / ARGUMENT-NUMBER /
> ARGUMENT-VALUE / ENVIRONMENT-VALUE / ESCAPE KEY / CRT STATUS` (com os registros
> emparelhados `DISPLAY … UPON ARGUMENT-NUMBER / ENVIRONMENT-NAME`).
> **Resolvido (1.7.2):** `OPEN … SHARING/WITH LOCK`, `READ … WITH [NO] LOCK`,
> `UNLOCK` (liberta os bloqueios de registro INDEXED) e `CANCEL program`.
> **Resolvido (1.8.0):** `COMMIT` / `ROLLBACK` como transações de arquivos INDEXED
> controladas pelo programa (motores de memória e disco; registro de desfazer real em
> disco).
