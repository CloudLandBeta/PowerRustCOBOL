...
ウィンドウを持つアプリケーションには端末が結び付いていません。そのため IDE が
死ぬと、パニックのメッセージも `file:line` もバックトレースも、誰も読んでいない
stderr へ流れていきます。ウィンドウはただ消え、後には何も残りません。これを
2 つの異なる仕組みで置き換えます。別々の問題を解くものだからです。

**クラッシュログ — 診断する材料を残すために。** パニックフックが
`<data>/cobolt/crash/crash-<秒>.log` を書き出します。内容はパニックのメッセージ、
`file:line:column`、強制取得したバックトレース、IDE のバージョン、OS、スレッド、
そしてそのとき開いていたファイルです。バグ報告に添付してください。

**自動保存 — 作業を生き延びさせるために。** **20 秒**ごとに、未保存のエディタ
バッファと変更された form が `<data>/cobolt/recovery/` へコピーされます。各コピー
を元のファイルに対応付ける `manifest.toml` も一緒に置かれます。セッションが動作中
であることを示すマーカーファイルは、正常終了時に削除されます。次回の起動時に
それが残っていることが、まさに「前回のセッションは正常に終わらなかった」という
意味であり、IDE はそこで復旧を提案します。

**復旧は決して上書きしません。** 提案を受け入れると、各コピーは元のファイルの隣に
`<名前>.recovered.<拡張子>` として書き出され、そのパスが Output パネルに一覧され
ます。そのコピーは、すでに足場を失っていたプロセスから出てきたものです。どちらの
版を採るかを決めるのは IDE ではなく、あなたです。

> ⚠️ **パニックフックですべてを捕まえることはできません。** スタックオーバー
> フローはガードページでフォールトし `SIGSEGV` として届きます。メモリ不足キラーは
> `SIGKILL` を送ります。巻き戻し中の二度目のパニックは abort します。いずれの
> 場合もフックは実行されず、**クラッシュログは書かれません**。これらを覆うのが
> 自動保存です。何かが起きた時点では、すでに保存が済んでいるからです。だからこそ、
> 実際の保証はこの間隔そのもの、すなわち最大 20 秒ぶんの作業ということになります。
