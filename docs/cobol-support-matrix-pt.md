...
|---|---|
| [`cobol85-supported-syntax-en.md`](cobol85-supported-syntax-en.md) | **Que grafia** de cada instrução é realmente aceite pelo lexer, pelo analisador e pelo ambiente de execução, e o marcador de conformidade NIST CCVS85 |
...
| **20xx** | Definido por uma **norma ISO posterior** — COBOL 2002 / 2014 / 2023, e o que está a ser redigido rumo a 2026 |
...
previsto, ainda por implementar · `🚫` fora de âmbito por desenho, nunca será
implementado.
...
> implementação certificada do COBOL-85. A conformidade é *medida* contra a suite
...
|---|:--:|:--:|:--:|:--:|---|
| Fonte em formato fixo, **relaxado** (`fixed-relaxed`) | ● | ○ | ○ | ✅ | **O valor por omissão.** A área de sequência e a coluna indicadora são respeitadas, mas a linha vai até onde o programador escreveu — sem corte na coluna 72. Os `.cbl` gerados dos formulários e os blocos `EXEC RUST` precisam disto |
| Fonte em formato fixo, **formato de referência clássico do COBOL-85** (`--source-format=fixed`) | ● | ○ | — | ✅ | Todas as regras de colunas aplicadas: 1–6 sequência, 7 indicador (`*` `/` comentário, `-` continuação, `D` linha de depuração), 8–72 fonte, **73–80 descartadas**, e a junção padrão das continuações, incluindo um literal alfanumérico continuado. É nisto que está escrita a suite de imagens de cartão NIST CCVS85. **Escolhido explicitamente, nunca por deteção** — aplicar estas regras a fonte que não foi escrita para elas apaga código em silêncio |
...
| PROCEDURE DIVISION | ● | ○ | — | ✅ |  |
| Programas aninhados | ● | ○ | — | ✅ |  |
| Várias unidades de programa sequenciais num só ficheiro | ● | ○ | — | ✅ |  |
...
| LINKAGE SECTION | ● | ○ | — | ✅ |  |
| FILE SECTION | ● | ○ | — | ✅ |  |
| SCREEN SECTION | ● | ○ | — | 🚧 | Os `ACCEPT`/`DISPLAY` estendidos com `AT`/`WITH` executam via ANSI em modo CLI; a edição de ecrã campo a campo é substituída pelo desenhador visual de formulários em modo GUI |
| COMMUNICATION SECTION (`CD`, controlo de mensagens) | ● | — | — | 🚫 | Teleprocessamento; obsoleto nas normas posteriores |
| REPORT SECTION / REPORT WRITER (`RD`, `GENERATE`) | ● | ○ | — | 🚫 | Fora de âmbito por desenho |
...
| `CANCEL` | ● | ○ | — | ✅ |  |
| `INVOKE` | — | ● | ○ | 🚧 | OO do COBOL 2002. Suportado para **objetos de GUI e de execução e para plug-ins de FFI de Rust**; as definições de classe e método pelo utilizador não estão implementadas |
| `UNLOCK` | — | ● | — | 🚧 | Conduz os bloqueios de registo dentro da execução; não é imposto entre processos do sistema operativo |
| `COMMIT` / `ROLLBACK` | — | — | ● | ✅ | Transações controladas pelo programa sobre ficheiros INDEXED, com um registo de desfazer a sério |
...
| Expressões de propriedade concisas (`Output::Value`) | — | — | ● | ✅ | Ler ou definir uma propriedade de um controlo dentro de uma fórmula, sem qualquer item temporário de working-storage |
...
controlo. Nada disto é COBOL normalizado.
...
| `SET target TO <expression>` | — | — | ● | ✅ | Equivalente à forma `COMPUTE`; o destino pode ser um item de dados ou uma propriedade de controlo como lvalue |
...
| **Inferência de tipos** — ler `Ctrl::Property` dá um valor tipado de primeira classe | — | — | ● | ✅ | O tipo numérico ou de texto flui pela expressão, pelo que uma propriedade entra diretamente numa operação aritmética, numa condição ou numa posição emissora **sem qualquer item `PIC` pelo meio**: `IF Slider-1::Value > 50`, `COMPUTE Total-Lbl::Value = Qty-Box::Value * Price-Box::Value`. O valor de uma propriedade que parece numérico é lido de volta como numérico, de modo que as comparações e a aritmética se mantêm algébricas e não caráter a caráter |
...
## 6. E/S de ficheiros — organizações e acesso

| Capacidade | 85 | 20xx | PRC | Estado | Notas |
|---|:--:|:--:|:--:|:--:|---|
| `ORGANIZATION IS SEQUENTIAL` | ● | ○ | — | ✅ | Registos de comprimento fixo |
...
| `ORGANIZATION IS RELATIVE` | ● | ○ | — | ✅ | Motor próprio (`cobolt-runtime/src/relative.rs`, contentor `PRCREL1`, disco e MEMORY). O `RELATIVE KEY IS` endereça os registos por número inteiro de registo a partir de 1; os três modos de acesso; os sete verbos de ficheiro despacham sobre ele. **O módulo RL do NIST está terminado nos dois eixos** — 35/35 de compilação, 34/34 de execução, 354 asserções, 0 falhas (motor 1.62.76, módulo 1.62.77) |
...
| `OPEN … WITH LOCK` (abrir o ficheiro em exclusivo) | — | ● | — | 🚧 | O mesmo: aceite e consultivo no modelo de unidade de execução única |
| `READ … WITH LOCK` | — | ● | — | ✅ | O motor já detém o registo sob `I-O`; a frase declara a intenção |
...
| Partilha de ficheiros entre processos e imposição de bloqueios de registo | — | ● | — | ⛔ | Previsto; hoje o modelo é de uma única unidade de execução |

## 7. E/S de ficheiros — o motor INDEXED (PowerRustCOBOL)
...
| Capacidade | 85 | 20xx | PRC | Estado | Notas |
|---|:--:|:--:|:--:|:--:|---|
| `STORAGE [MODE] IS DISK` | — | — | ● | ✅ | **O modo de armazenamento por omissão.** Os registos e os índices vivem no ficheiro do `ASSIGN` e são lidos a pedido, pelo que a RAM se mantém limitada mesmo em ficheiros muito grandes. Servido pelo motor redb à prova de falhas desde a 1.62.73; ao B+tree paginado mais antigo ainda se chega com `--indexed-engine rust` |
| `STORAGE [MODE] IS MEMORY` | — | — | ● | ✅ | O ficheiro inteiro em RAM, persistido no caminho do `ASSIGN` ao fechar |
| `WITH [DATA] COMPRESSION` | — | — | ● | ✅ | RLE sem dependências; esmaga bem acima dos 50 % as sequências de enchimento típicas dos registos COBOL |
| `COMMIT` / `ROLLBACK` controlados pelo programa | — | — | ● | ✅ | Registo de desfazer a sério, nos motores de memória e de disco |
| Bloqueio de registos dentro de uma unidade de execução | — | ○ | ● | ✅ | Ver a ressalva sobre processos acima |
| Motor selecionável (`--indexed-engine rust\|rm-cobol85\|fujitsu\|redb`) | — | — | ● | ✅ | Também `COBOL_INDEXED_ENGINE`; todos compatíveis em comportamento. **O `redb` é o valor por omissão** desde a 1.62.73 (`cobolt-runtime/src/indexed.rs:126`) |
| Motor ACID `redb` à prova de falhas | — | — | ● | ✅ | OPEN em O(1) (~5 ms com 200 k registos), RAM do conjunto de trabalho (≥250 M registos), sobrevive a uma falha de energia sem corromper o índice |
| Contentor autodescritivo `PRCIDX1` | — | — | ● | ✅ | Embute o formato do registo e os descritores de chave; a validação estrita na abertura converte uma divergência de esquema em `39` e um ficheiro ausente em `35`. Não é compatível byte a byte com a Fujitsu |
| Registo de transações por ficheiro (`--indexed-log basic\|full`) | — | — | ● | ✅ | logfmt ou NDJSON pronto para Grafana/Loki — ver [`observability-pt.md`](observability-pt.md) |
...
| Capacidade | 85 | 20xx | PRC | Estado | Notas |
|---|:--:|:--:|:--:|:--:|---|
| **SQL** — SQLite, PostgreSQL, MySQL | — | — | ● | ✅ | Uma mesma superfície de CALL para os três; o backend é escolhido a partir da cadeia de ligação. **Sem bibliotecas do sistema** — nada é ligado a partir do anfitrião — mas «Rust puro» só é verdade para dois dos três: o `postgres` e o `mysql` são-no, enquanto o `rusqlite` está fixado com `features = ["bundled"]` e compila a **amálgama em C do SQLite** através do `libsqlite3-sys`. (Essa compilação de C é também a razão por que o `test_external_crates_e2e` falha de forma intermitente dentro de um `cargo build` aninhado.) Ver [`database-runtime-pt.md`](database-runtime-pt.md) |
| **Conjuntos de resultados SQL** — `Fetch()`, `ColumnNames()`, `ColumnCount()`, `ColumnName(n)` | — | — | ● | ✅ | O `Fetch()` devolve a linha seguinte separada por tabulações e vazia quando se esgota, pelo que termina o seu próprio ciclo; o `ColumnNames()` nomeia o conjunto de resultados pela ordem do SELECT, mesmo quando não correspondeu a nenhuma linha. A superfície de `CALL` lê antes a linha atual coluna a coluna por índice — as duas travessias não devem ser misturadas no mesmo identificador |
...
| **Ficheiros de texto** — `COBOL-APPEND-FILE`, `COBOL-WRITE-FILE` | — | — | ● | ✅ |  |
| **Temporizadores** | — | — | ● | ✅ |  |
...
| **Procedimentos do utilizador** | — | — | ● | ✅ | Procedimentos COBOL partilhados, editáveis no IDE e chamáveis como `CALL "PROCEDURE-NAME"` |

## 9. Explicitamente fora de âmbito
...
| Capacidade | 85 | 20xx | PRC | Estado | Notas |
|---|:--:|:--:|:--:|:--:|---|
| COMMUNICATION SECTION (`CD`, controlo de mensagens / teleprocessamento) | ● | — | — | 🚫 | Obsoleto nas normas posteriores; sem uso moderno |
...
| Controlos ActiveX / OLE / COM | — | — | — | 🚫 | Específicos de uma plataforma e não portáveis |
...
|---|:--:|---|
| Desenhador visual de formulários | ✅ | Tela de desenho com vários temas (**Liquid Glass**, **Cobalt Steel**), ajuste à grelha, redimensionamento por arrasto de controlos e da tela, alinhamento com seleção múltipla e ordenação em profundidade |
...
| Catálogo de controlos | ✅ | **43 widgets** distribuídos por Common, Container, Data, Graphics, Menu, Non-visual e Charts, mais um tipo `Custom` fornecido por plug-ins |
...
| `Transparency` por controlo | ✅ | 0 = opaco … 100 = transparente; esbate a face, a moldura e a sombra enquanto o texto, os glifos e a borda se mantêm legíveis. As legendas que ficam abaixo do WCAG AA face ao que têm por trás saltam para o polo que se lê |
...
| Ligação de dados e matrizes de controlos | ✅ | Ligação direta a fontes SQL e de dados; os **grupos repetidores visuais** expandem matrizes de GroupBox e Panel a partir do número de linhas do `DataSource` em execução |
| Validação visual e inspetor de formulários | ✅ | Emblemas de erro em tempo real para manipuladores malformados, ligações incompletas e anomalias de disposição; o gestor de processos do `rcrun` acompanha ao vivo a percentagem de CPU, a RSS, os registos e o número de linhas de execução |
...
| Malha de assistentes de IA agêntica | ✅ | Orquestrador de LLM **rig-core** (Ollama, OpenAI, Groq, Alibaba Model Studio e outras API na nuvem) a correr o Dev Agent, o Editor Assistant e o History Compactor, com um registo de observabilidade ao vivo e leituras de tokens `↑input ↓output` |
...
| Base de conhecimento em pedaços com RAG | ✅ | Indexada com um registo por assunto; é distribuída já embebida, com GPU e um recuo para CPU que não aquece, e **File → Reindex Knowledge Bases** |
...
| Execução com várias janelas | ✅ | Ecrãs de pré-visualização e de execução em viewports próprios do sistema operativo (multi-viewport do egui) |
...
| Diálogos de ficheiro nativos e não bloqueantes | ✅ | Abrir, guardar e procurar sem travar o ciclo de eventos da interface |
