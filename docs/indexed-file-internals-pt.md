...
# Interioridades dos arquivos indexados do PowerRustCOBOL (motor paginado `PRCIDXD1`)

Este documento é um esquema conceptual do motor **persistente e paginado em
disco** que suporta os arquivos `ORGANIZATION IS INDEXED` declarados com
`STORAGE IS DISK` (o valor por padrão). É um desenho de B+tree com páginas
ranhuradas que lê os registros a pedido, para que a RAM fique limitada
independentemente do tamanho do arquivo.

> ⚠️ **Este já não é o motor por padrão.** O `STORAGE IS DISK` continua a ser o
> *modo de armazenamento* por padrão, mas desde a **1.62.73** o motor que o
...
> omissão.
>
> **Escopo.** Isto descreve o *motor físico* (`DiskIndexedFile`, número mágico do
> contentor `PRCIDXD1`). É um artefato diferente do contentor `PRCIDX1`, de
...
e a (des)materialização dos registros em
...
Um arquivo paginado com **uma página de cabeçalho + N B+trees (um por chave) →
um diretório de RecordId → páginas de dados ranhuradas com imagens de registro
posicionais e de largura fixa**, mais uma lista livre, cadeias de transbordo,
compressão RLE opcional e um registro de desfazer em memória para as transações.
...
uma lista livre, de modo que a ordem física das páginas em disco **não** acompanha a
...
| `3` | `PT_DATA`     | página ranhurada que empacota várias imagens de registro |
...
| `compressing`     | `1` se as cargas úteis dos registros são comprimidas com RLE em disco. |
...
chaves* (intervalos de bytes). Tudo o resto acerca de um registro é posicional —
ver a §6.
...
**Um registro, muitas chaves.** A chave primária e todas as alternativas apontam
para o *mesmo* RecordId, de modo que existe exatamente uma cópia guardada de cada
registro. Os índices alternativos são apenas B+trees adicionais sobrepostos ao
...
- As folhas estão **duplamente ligadas** (`next`/`prev`), de modo que um varrimento
...
- Cabeçalho de página de 5 bytes e, a seguir, um **diretório de ranhuras** que
  cresce a partir da frente, enquanto as **cargas úteis dos registros** crescem a
  partir do fim; um registro cabe em linha enquanto as duas regiões não se
  encontrarem.
- Uma ranhura é `(offset, len)`; apagar um registro põe a sua ranhura a `len = 0`
...
Um registro maior do que o limite em linha (`PAGE_SIZE − cabeçalho − uma ranhura`)
...
O diretório é mantido em RAM como um `Vec<RecLoc>` enquanto o arquivo está
...
guardam RecordId, nunca endereços físicos, de modo que um registro pode ser movido em
disco sem tocar em qualquer índice.

---

## 6. A própria imagem do registro (posicional, sem nomes)
...
- **A identidade é a posição.** Este é o caso-limite de «não repetir as chaves em
  ...
  chave reescreve apenas o esquema de chaves do cabeçalho, não os registros nem os
  índices. Alterar o deslocamento ou a largura de um campo é a única alteração que
  obriga a reescrever os dados — algo inerente aos registros de comprimento fixo
...
- **Lista livre.** O `free_list_head` encadeia páginas recuperadas de páginas de
  ...
  dela antes de incrementar o `next_page_id`, de modo que o espaço é reutilizado e o
  arquivo não cresce monotonicamente.
...
## 8. Transações (registro de desfazer em memória)

O motor de disco mantém um **registro de desfazer** com as inversas de cada
...
do programa (comprimento do registro, número de chaves, as partes de cada chave e a
sua política de duplicados). Uma divergência devolve o estado de arquivo COBOL
...
| Nomes dos campos de dados     | Apenas o `FD` do programa              | não está no arquivo |
| Bytes dos registros            | Páginas `PT_DATA` / `PT_OVERFLOW`       | uma por registro |
| chave → RecordId              | um B+tree por chave                    | um por chave |
| RecordId → localização física | Diretório de RecordId (cadeia `PT_DIR`) | uma por registro |
| Páginas livres                | Lista livre (`free_list_head`)         | —           |
| Inversas de alterações por confirmar | Registro de desfazer em RAM       | por transação |
