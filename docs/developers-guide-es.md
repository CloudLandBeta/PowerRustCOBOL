...
8. [El control catalogue](#8-el-control-catalogue)
...
### La comprobación de Rust en el primer arranque

La IDE diseña forms y *ejecuta* programas por sí sola. **Build** es la
excepción: compila tu project como aplicación nativa mediante el **toolchain de
Rust** (§18), igual que cualquier Run de un programa que contenga un bloque
`EXEC RUST`. Por eso, en su primer arranque PowerRustCOBOL busca Rust — y cuando
encuentra uno utilizable, no dice nada.

Cuando no lo encuentra, te indica en qué caso estás — Rust ausente, o una
versión anterior a la **1.92** que PowerRustCOBOL exige —, muestra el comando
oficial de [rustup.rs](https://rustup.rs) y se ofrece a ejecutarlo por ti. Si lo
rechazas, se te pregunta una segunda vez, porque rechazarlo tiene un precio que
conviene decir en voz alta:

| Sin Rust pierdes | Conservas |
|---|---|
| **Build** — ningún ejecutable nativo, nada que empaquetar | El Form Designer |
| Ejecutar cualquier programa que contenga un bloque `EXEC RUST` | El editor de código y las herramientas COBOL |
| | **Run** (interpretado) y el debugger |

Rechazarlo la segunda vez zanja el asunto y la pregunta no vuelve a aparecer.
Instala Rust más adelante desde [rustup.rs](https://rustup.rs) y **Build**
empezará a funcionar sin más — no hay que avisar a la IDE.

> **Nota** — rustup instala Rust en `~/.cargo/bin`, ruta que añade a `PATH` tu
> *perfil de shell*. Una aplicación iniciada desde el Finder o desde el
> escritorio de Windows nunca lee ese perfil, así que PowerRustCOBOL mira en esa
> ubicación por su cuenta y usa lo que encuentra. No necesitas lanzar la IDE
> desde un terminal para que **Build** funcione.
...
- **Main Pane (centre / right of the tree).** Muestra el code editor, el
  **property inspector** (al hacer clic en un form o control del tree), **o el
  ...
  controls para ancho consistente (sin falta de ~9px en el borde derecho) y
...
- **Toolbox (left).** Widgets agrupados en **Non-Visual**, **Common**,
  **Container**, **Data**, **Graphics**, **Menu**, **Charts** y **Dialogs**.
  Arrastra cualquier control al canvas.
...
- **Arrastra un lazo** sobre el lienzo vacío: se selecciona todo control que
  toque el rectángulo.
- **Mantén Command (macOS) o Control (Windows/Linux) y haz clic**: añade un
  control a la selección, o lo quita si ya estaba. Arrastrar con el modificador
  un control aún no seleccionado lo añade y mueve toda la selección en un solo
  gesto.

Seleccionar un **contenedor** selecciona con él a sus hijos a efectos de mover,
de modo que un GroupBox arrastra todo su subárbol y mantiene rígida su
disposición. El primer control seleccionado es el **primario**: los comandos de
alineación y tamaño se miden respecto a él, y el panel de propiedades lee sus
valores.

**Arrastrar una selección es rígido.** Todo el grupo se mueve con un único
desplazamiento, tomado del control que está bajo el puntero, así que la
separación que hayas dispuesto sobrevive al movimiento, incluso cuando los
controles no caen sobre las líneas de la cuadrícula.

**El panel de propiedades edita toda la selección.** Con más de un control
seleccionado muestra lo que tienen en común y aplica cada cambio a todos:

- **Mismo tipo**: el panel completo. Toda propiedad que tiene un Button la
  tienen cinco Buttons seleccionados.
- **Tipos distintos**: solo las propiedades que sus tipos comparten realmente,
  porque una fila que solo algunos tuvieran parecería funcionar y no cambiaría
  nada en los demás.

Una edición es **un solo paso de deshacer**, por muchos controles que haya
tocado. Los controles que no tienen la propiedad se dejan como están en lugar de
dársela, y la identidad —el ID del control, el orden de tabulación y el padre—
nunca se comparte, porque dos controles no pueden tener la misma.
...
> **Note (non-visual controls).** Timer, AI Agent, REST Client y SQL Database
...
> 📷 **Screenshot needed — `control-gallery.png`.** Un single form (o la preview
> window) mostrando uno de cada major control para que los newcomers los reconozcan.
...
> CheckBox, los items de un ListBox y el cursor de texto. Tu color se usa tal
> como lo fijaste mientras siga siendo legible ahí; donde no lo sería, el
> painter recurre a negro o blanco — el que se lea. Para fijar un color de forma
> absoluta, elige uno que se lea en el theme que distribuyes.
>
> **Contra qué superficie se mide cada uno.** Contra aquella sobre la que el
> texto cae realmente. Un CheckBox tiene dos superficies (ver abajo): el caption
> se apoya en el **marco** y se comprueba contra `BackgroundColor`, mientras que
> la marca `CheckColor` va dentro de la **casilla** y se comprueba contra
> `CheckBoxColor`. Dar un color de marco oscuro a un check box ya no vuelve
> blanca su marca, y colorear la casilla ya no vuelve blanco el caption.
>
> **Un marco transparente queda en tus manos.** En cuanto `Transparency` pasa de
> 70, el marco pinta demasiado poco para leerlo, y aquello sobre lo que el
> caption cae de verdad — el form, un GroupBox, una imagen de fondo — no es algo
> que el control pueda ver. Ahí no se mide nada y tu `ForegroundColor` se usa
> exactamente como lo fijaste. Un CheckBox es 100 % transparente por defecto,
> así que este es el caso normal: elige un color de caption que se lea sobre el
> form donde lo colocas.
>
> El caption de un CheckBox va a la derecha de su casilla, y el de un
> RadioButton a la derecha de su círculo de selección, a la misma distancia.

> **Un CheckBox tiene dos superficies, y cada una tiene sus properties.** Si
> vienes de PowerCOBOL o isCOBOL esperarás un solo background y un solo border;
> aquí la casilla es una superficie por derecho propio, así que hay dos de cada.
> Qué superficie nombra cada property nunca depende del control:
>
> | Superficie | Qué es | Sus properties |
> |------------|--------|----------------|
> | **Marco** | La tarjeta detrás del caption *y* de la casilla — todo el rectángulo del control | `BackgroundColor` (o el par de gradiente), `BorderStyle`, `BorderColor`, `BorderWidth` |
> | **Casilla** | La casilla de marcado en sí — el círculo de selección de un RadioButton | `CheckBoxColor`, `CheckBoxBorderStyle`, `CheckBoxBorderColor`, `CheckBoxBorderWidth` |
>
> `CheckColor` y `CheckSize` siguen siendo lo que siempre fueron: la marca
> dibujada *dentro* de la casilla, y cuánto de la casilla ocupa.
>
> Por tanto `BackgroundColor` significa en un CheckBox exactamente lo mismo que
> en un Label, un TextBox o un Panel: la cara del propio control. Un check box
> empieza 100 % transparente y con `BorderStyle` en `None`, así que el marco no
> muestra nada hasta que lo pidas; la casilla, en cambio, empieza con
> `CheckBoxColor` vacío, lo que la deja con lo que pinte el theme activo. Nombra
> un color y manda el tuyo.
>
> ```cobol
...
> ```
>
> ⚠️ **Advertencia.** Un border y una cara son decisiones distintas. Un control
> sin marco — un CheckBox dejado transparente, un Label sin background — dibuja
> igualmente el border que hayas pedido, sobre nada. Es deliberado: antes
> `BorderStyle` no tenía ningún efecto en esos dos, que es lo más sorprendente.
...
### Esquinas redondeadas y la sombra del control

`CornerRadius` redondea el fondo y el borde del control, y recorta el contenido
a la forma redondeada. El valor se limita a la mitad del lado menor, de modo que
nunca pasa de una forma completamente redondeada ("píldora" o círculo); `0`
significa esquinas rectas y ningún recorte.

La zona que el radio recorta deja de pertenecer al control. Lo que se ve ahí es
lo que está **detrás** de él: la superficie del formulario **y la sombra que el
propio control proyecta sobre ella**. Esa continuidad es lo que hace que un
control redondeado parezca apoyado sobre el formulario en lugar de recortado de
él, y se nota sobre todo con **Shadow distance** y **Shadow blur** generosos.

El mismo radio y el mismo recorte se aplican igual en el lienzo de diseño, en la
vista previa y en el formulario en ejecución.
...
> **runtime currently fires** form `onLoad`/`onClose`, y para controls:
...
y después de `OF` lista los **controls que realmente exponen esa property**
...
Properties describen *qué es un control*; **methods** describen *qué puede hacer* —
mostrarlo, moverlo, incrementar un value, agregar un list item, disparar un HTTP
request. Cada control entiende un set de **universal** methods más sus propios
...
de un control después de que escribes `::`, cada uno con una one-line description.

**Universal methods** (cada visible control):

| Method | Effect |
|--------|--------|
| `Show` / `Hide` | Define la property `Visible` on u off. |
| `Enable` / `Disable` | Define la property `Enabled` on u off. |
| `SetFocus` | Da keyboard focus al control. |
| `MoveTo(x, y)` | Reposiciona el control (define `X` / `Y`). |
...
> non-visual control (sus properties guardan la connection string, driver y los
...
- **HTTP/REST.** `COBOL-HTTP-GET/POST/PUT/DELETE` emiten requests;
  ...
  **REST Client** non-visual control te da un designable endpoint con events para
  responses, errors, timeouts y progress.
- **AI agents.** El **AI Agent** non-visual control modela una connection a un
...
- **Event firing.** Todos los form/control events son *designable*; solo el core set es
...
- **Control / control** — un element en un form (button, text box, chart, …).
...
- **Non-visual control** — un service sin run-time appearance (Timer, SQL,
  REST, AI Agent).
