...
- RELATIVE ファイルを使う 35 本のうち 30 本は、かつて runtime に
  **RELATIVE エンジンがまったくない**ままきれいにコンパイルできていました —
  実行すれば、黙って誤った結果を出していました。このギャップは**塞がれました**:
  エンジンは 1.62.76 で入り、モジュールは 1.62.77 で完了しました
  （コンパイル 35 / 35、実行 34 / 34）。コンパイルの軸だけでは分からないことを
  最もよく示す例として、ここに残してあります。
...
| RL | 相対入出力 | 35 / 35 | ✅ **両方の軸で完了**（1.62.77）— 実行 34 / 34、354 件のアサーション、失敗 0 件。本物のエンジン（`cobolt-runtime/src/relative.rs`、`PRCREL1` コンテナ）が 1.62.76 で入り、7 つのファイル動詞すべてが `FileOrganization::Relative` に基づいて処理を振り分けます。RL301M は IX301M と同じ裁定により実行からは除外されますが、コンパイルの集計には引き続き数えられ、そこでは合格しています |
...
3. **オブジェクト指向 COBOL**（クラス／メソッド定義）— `INVOKE` は COBOL の
   ...
   します）。
4. 認識されない組み込み関数名は依然として **0** を返します — 同じ静かな失敗の
   仕方です。仕様：
   [組み込み関数](../specs/nist/NIST-spec-intrinsic-function-gaps.md)。
5. ⚠️ **不正な `ACCESS MODE` ／ `ORGANIZATION` の値が診断なしに握りつぶされ
...
6. ⚠️ **`ALPHABET … IS EBCDIC` は受け付けられますが、ネイティブ（ASCII）の
