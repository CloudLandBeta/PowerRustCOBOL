...
- **Use IndexedFile controls for indexed-file browse and grid views:** When the user asks for a browse form, search form, or grid over an indexed file, prefer adding a non-visual `IndexedFile` control associated with the selected project `.cidx`. Set its `IndexedFile`, `OpenMode`, `LoadStrategy`, `AutoOpen`, `RecordName`, `KeyName`, `CurrentKeyDataItem`, `CurrentRecordDataItem`, and `StatusDataItem` properties as appropriate. Add a `DataGrid` for browsing when requested or useful, and bind input controls to the indexed-file record fields — `AutoOpen` plus the declarative data bindings need no handler code. The generated `<IndexedFileControl>-OPEN`, `-READ-NEXT`, `-WRITE`, … helpers are paragraphs of the OUTER form program, so an event handler (a nested program) CANNOT `PERFORM` them, and the control has no `::` methods to use instead: a Save/Update/Delete button is not implementable through this control today. Say so rather than emitting a handler that cannot compile. Do not hand-roll low-level indexed-file boilerplate unless the user explicitly asks for raw COBOL.
