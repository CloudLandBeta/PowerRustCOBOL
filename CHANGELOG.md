...
A developer writes COBOL in nine kinds of places — control event handlers,
form lifecycle handlers, user procedures, the five structure sections, and
Common Code files. Both halves of this feature stand on one new idea: a
**code site**, a stable address for each such place, plus a **source map**
from every generated `.cbl` line back to the site (and the line *within* it)
that produced it. The map is produced by the same call that produces the
source, and the generated bytes are pinned byte-identical by a golden test.

**Diagnostics say where.** A Check no longer reports `842:17` against a
generated artifact: the Output row reads
`MAIN-FORM ▸ BTN-OK ▸ onClick — 3:12: ✖ error: …` with the offending line
quoted and the column marked, and clicking it opens the owning editor — the
event modal, the COBOL Structure window, or the code editor — with the caret
on that line. The generated `.cbl` is never opened. A diagnostic on a line
codegen authored is labelled `[generated code]` and blamed on no site.

**Project-wide code search.** View ▸ Code Search…, the 🔍 toolbar button, or
Ctrl/Cmd+Shift+F (the in-tab Ctrl+F is untouched) opens a persistent,
non-blocking window: plain-text query over every code site of every form —
open forms read live, unsaved text — plus Common Code, with case-sensitive
and whole-word options, results grouped by form then site with matches
highlighted, and double-click to land on the line in the RAD. Generated
`.cbl` and the recycle bin are deliberately not searched. The window holds
its size (0.000 px drift across 120 frames under an overflowing result
list), resizes only by its grip, and closes only on ✕ or Cancel.

Internally, `agent_lint` now attributes defects through the map too — a
defect in a structure section is attributed to the operation that wrote it,
which the old nearest-`PROGRAM-ID` scan could not see at all.
