...
Every platform press shows its outcome — \"Copied 14 character(s) from TXT-1\", or the reason it failed — as a brief notice at the bottom of the form's own window, so a press never appears to do nothing. `copy`/`cut`/`paste` act on the text field that had focus WHEN the button was pressed (the press itself takes the field's focus away, so it is the field the user was in that counts), and an untouched field yields its designed text. A toolbar living in a SideMenu's footer panel carries out platform actions like any other.\n",
        "FileDropZone" => "\
