...
pub fn property_reference(name: &str) -> Option<(&'static str, &'static str)> {
    Some(match name {
        ...
        "SelectedItemId" => (
            "text (runtime-only)",
            "SideMenu: the id of the row the user last clicked — designed or added at run time — written just before `onMenuItemClick` fires. Writing it from COBOL highlights that row.",
        ),
        "ResponseBody" => (
...
fn control_purpose(name: &str) -> &'static str {
    match name {
        ...
        "MenuBar" => "Window menu bar (menu structure is edited in the designer and stored in a `.menu.yaml` sidecar, not in a property). Property `MenuBarStyle` (Free | Responsive, default Free) decides its width: Free is the width it was drawn at, Responsive runs the form's full width and follows a resize. Menu items may carry an icon from the built-in catalogue: 660+ pure-vector icons in 26 categories (documents, editing, navigation, commerce, payroll, receivables, payments, stock control, transportation, logistics, financial, company departments, transaction kinds, civilian vehicles, military equipment, and more). Icons are resolution-independent line work tinted by the item's colour; the engine can also apply a second accent colour, a drop shadow, or a neumorphic emboss.",
