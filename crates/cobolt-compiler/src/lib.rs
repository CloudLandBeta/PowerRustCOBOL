...
fn run_form_app(program: cobolt_ast::program::Program) {
    ...
    // Every embedded menu sidecar, registered for the shared painter. Only the
    // SideMenu branch below used to parse one, because a SideMenu is what puts
    // an application into shell mode; a MenuBar deliberately does not, so its
    // structure was never loaded and the bar painted "MenuBar (empty)" in a
    // built application exactly as it did under Run Form (operator,
    // 2026-09-06). The shell decision is unchanged — it still turns on the
    // SideMenu alone.
    cobolt_forms::paint::register_menus(MENUS.iter().filter_map(|(id, yaml)| {
        cobolt_forms::menu::parse_menu(yaml)
            .ok()
    ...
    let control_ids: Vec<String> = first_form.controls.iter().map(|c| c.id.clone()).collect();
    {
        ...
        std::thread::spawn(move || {
            ...
            // Spec 066 — each SideMenu's designed rows, so the program's own
...
pub fn property_reference(name: &str) -> Option<(&'static str, &'static str)> {
    Some(match name {
        ...
        "ViewMode" => (
            "`Full` | `Cards`",
            "Viewer, per view (`View1ViewMode`/`View2ViewMode`). `Full` shows the document under whichever `Layout` is active. `Cards` REPLACES it with a reflowing grid of one card per page. A card is the page **in miniature**, painted by the same renderer that paints the page, so its headings, tables and pictures are the page's own; a page the reader has not opened is not decoded (the control never reads a document it was not asked for) and its card shows a short extract and its number. One click selects a card, and a DOUBLE click opens it — leaving `Cards` for `Full` on that page, at its top. The grid's row AND column counts depend on the card size and **the control's own width** — never the window or the screen. Two toolbar buttons switch it, per view, so one side can browse cards while the other reads.",
        ),
        ...
        "BorderStyle" => (
            ...
            "Border drawing style, on every control that carries the property (Button included). `Single` is one line of `BorderWidth` in `BorderColor`; `Fixed3D`/`Raised` draw a relief lit from the top-left (lighter top and left, darker bottom and right); `Sunken` inverts it. ALL of them follow the control's `CornerRadius`, meeting halfway round each corner arc, and all draw the same whatever paints the face — glass, background gradient, form theme or asset pack. Under the Neumorphic glass style the relief comes from the form's own shadow stack instead: `Fixed3D`, `Raised` and `Single` all read as RAISED there, and `Sunken` turns that relief over (shadow top-left, highlight bottom-right) so the control reads as pressed IN. Raised or pressed is the whole of `BorderStyle` under Neumorphic.",
        ),
        ...
        "ShowIcons" => (BOOL_DOMAIN, "Draws an icon on every node, from the platform's own catalogue — the same one menus and toolbars use. On by default. A node names its OWN icon after a TAB in its `Items` line (`Warehouse\\tbox`); nodes that name none take `ParentIcon`/`ParentIconOpen`/`LeafIcon`."),
        ...
        "HotTrackColor" => (COLOR_DOMAIN, "The band behind the node under the pointer, when `HotTracking` is on. Empty is half the selection's weight, so the two are never confused."),
        ...
        "ClickedRow" => ("integer (runtime-only, read-only)", "DataGrid: the data row a click (or double-click) landed on, numbered from 1 as `GetCellValue` counts — written just before `onCellClick` / `onCellDoubleClick`, so the handler knows which row's button was pressed."),
        "ClickedColumn" => ("integer (runtime-only, read-only)", "DataGrid: the data column a click landed on, numbered from 1 (its position in `Columns`); 0 for a column with no data behind it. Written just before `onCellClick` / `onCellDoubleClick`."),
        ...
        "SplitPosition" => ("0–100", "Where the division line sits, as a PERCENTAGE of the splitter's inner width (Horizontal) or height (Vertical). 0 closes pane 1, 100 closes pane 2; both are legal."),
        "LineSize" => ("points 1–40", "Thickness of the division line."),
        "GripStyle" => ("one of: `FilledPill` | `HollowPill` | `FilledCircle` | `HollowCircle`", "How the drag grip on the division line is drawn."),
        "GripSize" => ("points 0–400", "The grip's extent along the line (pill length / circle diameter). 0 hides the grip; the line stays draggable."),
        "GripColor" => (COLOR_DOMAIN, "Grip colour. Empty = the division line's colour."),
        "ResizeBehavior" => ("one of: `Translate with divider` | `Scale within the pane` | `Anchor to the outer edge`", "On a SPLITTER PANE only: what that pane does with the controls inside it when the division line moves. `Translate with divider` (the default) keeps every control at the same distance from the LINE, in both panes, so both halves travel with it and a control can be carried past its pane edge and clipped. `Scale within the pane` keeps each control at the same FRACTION across the pane — growing the pane spreads its contents, shrinking packs them — and never scales sizes, so nothing is distorted or lost. `Anchor to the outer edge` keeps each control at the same distance from its pane's leading edge: pane 1's never moves so its contents stay put, pane 2's IS the line so its contents follow. Set per pane, so the two halves can differ. Dragging the division in the RAD rewrites the children's X/Y for real, as one undo step."),
        ...
        "AgentAPI" => ("a provider id — `openai` | `anthropic` | `cohere` | `gemini` | `perplexity` | `mistral` | `groq` | `openrouter` | `huggingface` | `together` | `deepseek` | `alibaba` | `xai` | `voyageai` | `ollama` | `ollama_cloud` | `llamafile` (the IDE's Model Providers list) — or `LMStudio` | `Custom`", "Provider. A provider from the list is addressed the way the IDE addresses it: Anthropic at `<API root>/messages`, Ollama at its native `/api/chat`, every other one at `<API root>/chat/completions`, where a stored full request URL or a bare origin is brought back to the API root."),
        "AgentAPIKey" => (
        ...
        "AgentEndpoint" => ("full URL, a path starting with `/`, or empty", "Overrides the provider's default endpoint. A full URL is used as it stands; a path (`/v1/chat/completions`) is joined onto `AgentURL`'s scheme and host."),
        ...
        "Stream" => ("retired", "**Retired.** It was seeded true on every AgentObject and never read. No longer seeded or shown; a value in an older form, or a COBOL read or write of it, is kept and ignored — it does NOT stream. Streaming is `StreamReply`."),
        "StreamReply" => ("true | false (default false)", "AgentObject: show the reply while the model writes it. The request asks the provider for a streamed reply (OpenAI-compatible and Anthropic SSE, Ollama NDJSON); up to ten times a second `PartialReply` receives the text so far and `onPartialReply` fires. The Ask ends exactly as an unstreamed one: `LastReply`/`Result`/`ResponseDataItem` get the whole text and `onResponse` fires once (or `onError`). `Busy` stays true throughout, and `TimeoutSeconds` becomes a SILENCE limit — each piece that arrives restarts it, so a long answer is never cut off. An Ask that offers tools is never streamed. A provider or `Custom` endpoint that cannot stream fails the Ask with its own error; turn the switch off for it."),
        "PartialReply" => ("text (runtime-only, read-only)", "AgentObject: with `StreamReply` on, the reply received so far — written just before each `onPartialReply`. Emptied when an Ask starts. The finished reply is `LastReply`, in `onResponse`."),
        "ReplyPiece" => ("text (runtime-only, read-only)", "AgentObject: with `StreamReply` on, only the text that arrived since the previous `onPartialReply` — the piece to hand a Viewer's `AppendToMessage`. The pieces, in order, add up to `PartialReply`. Emptied when an Ask starts."),
        "TimeoutSeconds" => ("seconds > 0", "Request timeout."),
        "TargetControls" => ("retired", "**Retired.** An agent never writes a control itself — its tools are the program's own, answered by handlers — so there was nothing to restrict. No longer seeded or shown; a value in an older form, or a COBOL read or write of it, is kept and ignored."),
        ...
        "RequestDataItem" => ("COBOL data-item name", "RestClient: when a POST/PUT/PATCH is called with no body argument, the value of this item is sent as the body."),
        "StatusDataItem" => ("COBOL data-item name", "RestClient: receives every request's HTTP status (0 when no response came), from the ::Get/Post/Put/Delete/Call verbs, Sync or Async. IndexedFile: receives the engine's own FILE STATUS after every facade operation (00, 10, 22, 23, 35, 39 …)."),
        ...
        "TimeoutMs" => ("milliseconds ≥ 0", "How long a request may take, in either Mode — Async or Sync. 0 falls back to `TimeoutSeconds` where the control has it (a RestClient); a WebSearch has none, so there 0 means no timeout."),
        ...
        "RuntimeRows" => (
            "JSON (runtime-only, written by the SideMenu's row methods)",
            "SideMenu: the rows the program added at run time, after the designed ones. Never set it by hand — use `AddItem`, `RemoveItem`, `SetItem…`, `AddSection` and `Clear`, which keep it well-formed and refuse edits to designed rows. Never saved in the `.cfrm`.",
        ),
        "SelectedItemId" => (
            "text (runtime-only)",
            "SideMenu: the id of the row the user last clicked — designed or added at run time — written just before `onMenuItemClick` fires. Writing it from COBOL highlights that row. MenuBar: the id of the item chosen by a click or its accelerator, written just before `onMenuClick` fires.",
        ),
        "ResponseBody" => (
        ...
        "ResultSetDataItem" => ("COBOL data-item name", "An item the program declares that receives each row `Fetch()` returns (tab-separated), and spaces once the rows run out."),
        ...
        "LoadStrategy" => ("retired", "**Retired.** The indexed file's definition (.cidx) decides its storage for every program on that file. No longer seeded or shown; a form that has it still gets its `WS-<id>-LOAD-STRATEGY` item, so code reading it compiles."),
        ...
        "CurrentRecordDataItem" => ("COBOL data-item name or empty", "Every facade READ also lands the record here (`READ … INTO`). The program declares the item."),
        "OperatorName" => ("a name, a quoted literal, or a data-item name; or empty", "The operator recorded by `OPEN … REGISTERED USER`. A plain name is sent as a literal; the name of an item the form declares is sent as that item."),
        ...
        "ShowTooltips" => (BOOL_DOMAIN, "In the running form, the bar, point or slice under the pointer shows `label: value` in a tooltip. Live data only — the sample a chart shows before it has any is not data. On by default."),
        ...
        "AnimationDuration" => (
            ...
            "How long an `AnimateValues` move, and the `AnimateOnLoad` growth, take — for the WHOLE series rather than per point. Default 2000. Anything below 250 is raised to 250 — under that the eye reads a jump rather than a movement, and the property would be honoured in name only.",
        ),
        ...
        "WarningColor" => (COLOR_DOMAIN, "The Gauge zone from the warning mark to the critical one. Empty = the built-in `#F57C00` amber."),
        ...
        "BreadcrumbBackgroundColor" => ("hex color string `\"#RRGGBB\"` or `\"#RRGGBBAA\"`, or empty", "SideMenu only. Background of the breadcrumb frame the sidebar owns. Empty (the default) follows the ContentPane's backdrop; alpha is accepted but the frame is still painted opaque."),
        "BreadcrumbFontSize" => ("points 4–200, or 0", "SideMenu only. Text size of the breadcrumb chain. 0 (the default) follows the sidebar's FontSize; any other value sets the chain alone without moving the menu labels."),
        "BreadcrumbHeight" => ("points ≥ 16 (default 28)", "SideMenu only. Height of the breadcrumb frame. Independent of the font: text too big is clipped by the frame, and a form loaded into the ContentPane starts below it."),
        "BreadcrumbIconSize" => ("points 8–200, or 0", "SideMenu only. Size of the Open/Collapsed toggle at the head of the breadcrumb, never taller than the frame. 0 (the default) keeps it a square of the frame's height, capped at 48."),
        "BreadcrumbTextAlign" => ("one of: `Top` | `Middle` | `Bottom`", "SideMenu only. Places the breadcrumb chain and its toggle, as one group, against the top, in the middle (default) or against the bottom of the frame."),
        "Collapsed" => ("`1` (true) or `0` (false)", "SideMenu only. The pane state the application opens in: collapsed to the icon rail or open (default). At run time the operator's own remembered choice wins over it."),
        "CornerRadiusBottomLeft" => ("pixels ≥ 0, or -1", "Snackbar: the notification's bottom-left corner radius. -1 (default) takes CornerRadius (12)."),
        "CornerRadiusBottomRight" => ("pixels ≥ 0, or -1", "Snackbar: the notification's bottom-right corner radius. -1 (default) takes CornerRadius (12)."),
        "CornerRadiusTopLeft" => ("pixels ≥ 0, or -1", "Snackbar: the notification's top-left corner radius. -1 (default) takes CornerRadius (12)."),
        "CornerRadiusTopRight" => ("pixels ≥ 0, or -1", "Snackbar: the notification's top-right corner radius. -1 (default) takes CornerRadius (12)."),
        "FooterHeight" => ("points ≥ 0 (default 72)", "SideMenu only. Height of the footer band; the footer Panel (`<id>-Footer`) is re-pinned to it on every change. 0 removes the footer."),
        "FullHeight" => ("`1` (true) or `0` (false)", "SideMenu only. True (default): the sidebar spans the window's whole height, its Y and Height are inert, and the breadcrumb starts at its right edge. False: the breadcrumb spans the full width and the sidebar fills the height beneath it."),
        "HeaderHeight" => ("points ≥ 0 (default 120)", "SideMenu only. Height of the header band that carries the logo (HeaderImage, or HeaderIcon when collapsed). Clicking anywhere in it folds or unfolds the rail; it is never resized at run time."),
        "HeaderIcon" => ("image path or empty", "SideMenu only. The mark shown 45x45, centred, in the header of a COLLAPSED rail. Empty draws the fold/unfold arrow instead, so the rail can always be reopened."),
        "IconEffect" => ("one of: `None` | `Shadow` | `Neumorphic`", "SideMenu only. How menu-item icons and the breadcrumb toggle are painted: flat (None, the default), with a drop shadow, or with neumorphic relief."),
        "Margin" => ("pixels ≥ 0 (default 16)", "Snackbar: distance between the stack and the edges of the form's surface at StackAnchor."),
        "Size" => ("one of: `Small` | `Medium` | `Large`", "Snackbar: the size class. Sets the width range (220–420 / 280–520 / 320–620), padding, default icon size (18/22/26), button height and how many text lines fit (1/2/3). Default Medium."),
        "TextWrap" => ("`1` (true) or `0` (false)", "Snackbar: wrap long text onto further lines, up to the Size class's line budget (1/2/3); text that still does not fit ends in an ellipsis. Off keeps a single line. Default on."),
        "Timeout" => ("milliseconds, -1 or 0", "Snackbar: how long a notification stays up, counted from when it BECOMES VISIBLE. -1 (default) takes the Category's (Info 4000, Question 6000, Warning 6000, Error 8000, Critical 0); 0 keeps it until dismissed."),
        "View1CardSize" => ("`0`–`100` (percent)", "Viewer. Card size for view 1 in `Cards` mode. Default 55; larger means fewer, bigger cards per row. The bottom-right slider drives it in `Cards` mode and `Zoom` in `Full`. The unprefixed `CardSize` is an alias for it."),
        "View1FindOpen" => ("`1` (true) or `0` (false)", "Viewer. Whether view 1's Find bar is open. Default off; Esc closes it. The unprefixed `FindOpen` is an alias for it."),
        ...
        "View1ShowFilmstrip" => ("`1` (true) or `0` (false)", "Viewer. Docks a strip of page thumbnails beside view 1's document. Default off; not drawn while the view is in `Cards` mode. The unprefixed `ShowFilmstrip` is an alias for it."),
        "View1Source" => ("file path or URL", "Viewer. The document view 1 shows. Setting it starts an asynchronous load (onLoadProgress, then onLoaded or onError); the format is detected from the extension and content. The unprefixed `Source` is an alias for it."),
        "View1ViewMode" => ("one of: `Full` | `Cards`", "Viewer. What view 1 shows: the document (`Full`, the default) or a grid of page cards (`Cards`). An unrecognised value reads as `Full`. The unprefixed `ViewMode` is an alias for it."),
        "View1Zoom" => ("percent, 25–1600", "Viewer. View 1's zoom. Default 100; values outside the range are clamped. Changed by the wheel, a double-click and the slider. The unprefixed `Zoom` is an alias for it."),
        "View2CardSize" => ("`0`–`100` (percent)", "Viewer. Card size for view 2 in `Cards` mode. Default 55; larger means fewer, bigger cards per row. The bottom-right slider drives it in `Cards` mode and `Zoom` in `Full`. Meaningful only when `SplitMode` is `LeftRight` or `TopBottom`."),
        "View2FindOpen" => ("`1` (true) or `0` (false)", "Viewer. Whether view 2's Find bar is open. Default off; Esc closes it. Meaningful only when `SplitMode` is `LeftRight` or `TopBottom`."),
        ...
        "View2ShowFilmstrip" => ("`1` (true) or `0` (false)", "Viewer. Docks a strip of page thumbnails beside view 2's document. Default off; not drawn while the view is in `Cards` mode. Meaningful only when `SplitMode` is `LeftRight` or `TopBottom`."),
        "View2Source" => ("file path or URL", "Viewer. The document view 2 shows. Setting it starts an asynchronous load (onLoadProgress, then onLoaded or onError); the format is detected from the extension and content. Meaningful only when `SplitMode` is `LeftRight` or `TopBottom`."),
        "View2ViewMode" => ("one of: `Full` | `Cards`", "Viewer. What view 2 shows: the document (`Full`, the default) or a grid of page cards (`Cards`). An unrecognised value reads as `Full`. Meaningful only when `SplitMode` is `LeftRight` or `TopBottom`."),
        "View2Zoom" => ("percent, 25–1600", "Viewer. View 2's zoom. Default 100; values outside the range are clamped. Changed by the wheel, a double-click and the slider. Meaningful only when `SplitMode` is `LeftRight` or `TopBottom`."),
        _ => return None,
    })
}
...
mod resolve_main_tests {
    ...
    /// Spec 066 — the built binary hands the interpreter every designed menu
    /// before the program runs, as `rcrun run-form` and the form host do.
    #[test]
