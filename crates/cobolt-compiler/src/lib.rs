...
fn control_purpose(name: &str) -> &'static str {
    match name {
        ...
        "Splitter" => "A draggable BAR between two controls — NOT a container: the seeded 200x8 default is a rule, not a pane, and nothing is placed 'inside' a splitter. `Orientation` Horizontal makes a wide bar that splits top from bottom; Vertical makes a tall one that splits left from right. DRAGGING (1.61.163): grab it anywhere and it moves along its axis, handing the room one pane gives up to the other. Its panes are found by geometry — on each side of the bar, the NEAREST control whose span across the bar overlaps the bar's own, so a splitter only ever moves something it actually runs across, and never another splitter. `MinimumSize` (default 25) is how small either pane may get and clamps the drag. `SplitPosition` is kept in step with the bar's position, so a handler can read where the operator put it. Before 1.61.163 the control took no interaction at all: it painted a slab, the grip could not be grabbed and nothing resized. STILL NOT: `AllowEdit` (no in-place rename surface) — never tell a developer it works.",
