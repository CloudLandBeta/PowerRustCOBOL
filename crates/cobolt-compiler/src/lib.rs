...
pub fn property_reference(name: &str) -> Option<(&'static str, &'static str)> {
    Some(match name {
        ...
        "ForegroundColor" => (COLOR_DOMAIN, "Text / foreground drawing color. On a CheckBox, RadioButton or DateTimePicker it is kept only while it reads on the surface the text lands on, and otherwise flips to black or white — measured against the control's FRAME, never against the tick box or circle its BackgroundColor paints. Above Transparency 70 the frame paints too little to measure, so the color is used exactly as set; a CheckBox is 100 % transparent by default, so its caption color is always the one you gave it."),
...
fn control_purpose(name: &str) -> &'static str {
    match name {
        ...
        "CheckBox" => "Boolean on/off box with caption. BackgroundColor paints the TICK BOX; the caption stands beside it on the control's frame, which is 100 % transparent by default — so a dark BackgroundColor colours the box and leaves the caption's ForegroundColor exactly as set.",
