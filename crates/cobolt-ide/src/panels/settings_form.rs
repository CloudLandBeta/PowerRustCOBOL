                    ...
                    ui.vertical(|ui| {
                        ...
                        ui.horizontal_top(|ui| {
                            ...
                            ui.scope_builder(egui::UiBuilder::new().max_rect(left_rect), |ui| {
                                ...
                                ui.set_min_width(splitter);
                                ui.add_space(property_indent);
                                ui.add(egui::Label::new(tr.set_focus_ring).truncate());
                            });
                            ui.allocate_space(egui::vec2(resizer_width, 0.0));
                            ui.add_space(gap_after_resizer);
                            ui.horizontal(|ui| {
                                ...
                                let mut colour = if self.draft.focus_ring_color.trim().is_empty() {
                                    ...
                                    crate::panels::properties::hex_to_color32(
                                        &self.draft.focus_ring_color,
                                    )
                                };
                                if crate::panels::properties::color_edit_button_closing(
                                    ui,
                                    &mut colour,
                                )
                                .changed()
                        ...
                        ui.horizontal_top(|ui| {
                            ui.add_space(splitter + resizer_width + gap_after_resizer);
                            ui.vertical(|ui| {
                                // ── Named REST connections ───────────────────────────
                                //
                                // These sit in the RIGHT column, past the resizer, like
                                // every other control. They were full width at first, and
                                // the pane's single continuous resizer line — which spans
                                // the whole content height by design — ran straight through
                                // the cards and their text (operator screenshot, 2026-09-08).
                                // Content conforms to the line: the line is what keeps
                                // every value control on one x.
                                ui.add_space(10.0);
