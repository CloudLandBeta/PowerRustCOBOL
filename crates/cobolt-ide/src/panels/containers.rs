...
//! Container/containment tree logic for the form designer (spec 012).
//!
...
pub fn ancestor_opacity(controls: &[Control], idx: usize) -> f32 {
    ...
    while let Some(pid) = controls[cur].parent.clone() {
        let Some(p) = index_of(controls, &pid) else { break };
        o *= cobolt_forms::paint::opacity_of(&controls[p]);
        cur = p;
    }
    o
}
...
mod tests {
    ...
    fn clip_and_opacity_compose() {
        let mut c = sample();
        // Btn is clipped to Pnl's content_rect.
        ...
        // Pnl opacity 50 → child ancestor_opacity 0.5.
    ...
    #[test]
    fn tab_visibility() {
        // TabControl with two children on different tabs.
        let mut c = vec![
        ...
        assert!(is_visible(&c, 1, &active), "tab-0 child visible when tab 0 active");
        assert!(!is_visible(&c, 2, &active), "tab-1 child hidden when tab 0 active");
    ...
    fn drop_target_rules() {
        ...
        // Dropping the Label over the Panel's content → Into Pnl (R8).
        assert_eq!(
            resolve_drop_target(&c, 100, 100, 2, &active),
            DropTarget::Into { container: "Pnl".into(), tab: None }
        );
        // Over the bare form → Form (R7).
        assert_eq!(resolve_drop_target(&c, 600, 400, 2, &active), DropTarget::Form);
        // Over the Button (a non-container) → adopt the Button's parent Pnl (R10).
        assert_eq!(
            resolve_drop_target(&c, 60, 58, 2, &active),
            DropTarget::Into { container: "Pnl".into(), tab: None }
        );
        // The Panel cannot be dropped into itself or a descendant (cycle guard).
    ...
    fn drop_rejects_inactive_tab_and_chrome() {
        ...
        let mut active = ActiveTabs::new();
        active.insert("Tabs".into(), 0);
        // A point on the tab strip (chrome, y<26) is not a content drop → Form.
        assert_eq!(resolve_drop_target(&c, 100, 10, 1, &active), DropTarget::Form);
        // A point in the content area drops into the active page.
        assert_eq!(
