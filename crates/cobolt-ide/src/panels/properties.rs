        ...
        section_card(ui, "form-sec-appearance", tr.sec_appearance, true, |ui| {
            egui::Grid::new("form_appearance")
                ...
                .show(ui, |ui| {
                    ...
                    // ── Neumorphic-specific controls (only shown for Neumorphic glass style) ──
                    if form.glass_style == cobolt_forms::model::GlassStyle::Neumorphic {
                        // The separator must occupy a full grid row of its own: a
                        // bare `ui.separator()` takes the first CELL of the next
                        // row, shifting the following label+widgets one column
                        // right — which pushed the illumination colour pickers
                        // into a clipped third column (label visible, no pickers).
                        ui.separator();
                        ui.separator();
                        ui.end_row();
                        ...
                        {
                            let mut c1 =
                                hex_to_color32(&form.neumorphic_params.illum_gradient_start);
                            let mut c2 = hex_to_color32(&form.neumorphic_params.illum_gradient_end);
                            ui.horizontal(|ui| {
                                if color_edit_button_closing(ui, &mut c1).changed() {
                                    action.form_props.push((
                                        "NeumorphicIllumGradientStart".into(),
                                        color32_to_hex(c1),
                                    ));
                                }
                                if color_edit_button_closing(ui, &mut c2).changed() {
                                    action.form_props.push((
                                        "NeumorphicIllumGradientEnd".into(),
                                        color32_to_hex(c2),
                                    ));
                                }
                            });
                        ...
                        ui.label("Shadow grad.");
                        {
                            let mut c1 =
                                hex_to_color32(&form.neumorphic_params.shadow_gradient_start);
                            let mut c2 =
                                hex_to_color32(&form.neumorphic_params.shadow_gradient_end);
                            ui.horizontal(|ui| {
                                if color_edit_button_closing(ui, &mut c1).changed() {
                                    action.form_props.push((
                                        "NeumorphicShadowGradientStart".into(),
                                        color32_to_hex(c1),
                                    ));
                                }
                                if color_edit_button_closing(ui, &mut c2).changed() {
                                    action.form_props.push((
                                        "NeumorphicShadowGradientEnd".into(),
                                        color32_to_hex(c2),
                                    ));
                                }
                            });
                        ...
                        ui.label("Illum. blur");
                        {
                            let mut v = form.neumorphic_params.illum_blur;
                            if ui
                                .add(DragValue::new(&mut v).speed(0.1).range(0.0..=8.0))
                                .changed()
                            {
                                action
                                    .form_props
                                    .push(("NeumorphicIllumBlur".into(), v.to_string()));
                            }
                        }
                        ui.end_row();
                        ...
                        {
                            let mut v = form.neumorphic_params.shadow_blur;
                            if ui
                                .add(DragValue::new(&mut v).speed(0.1).range(0.0..=8.0))
                                .changed()
                            {
                                action
                                    .form_props
                                    .push(("NeumorphicShadowBlur".into(), v.to_string()));
                            }
                        }
                        ui.end_row();
                        ...
                        {
                            let mut v = form.neumorphic_params.transparency as i64;
                            if ui
                                .add(DragValue::new(&mut v).speed(1).range(0..=100).suffix("%"))
                                .changed()
                            {
                                action
                                    .form_props
                                    .push(("NeumorphicTransparency".into(), v.to_string()));
                            }
                        }
                        ui.end_row();
                        ...
                        ui.label("Distance");
                        {
                            let mut v = form.neumorphic_params.distance;
                            if ui
                                .add(DragValue::new(&mut v).speed(0.25).range(0.0..=30.0))
                                .changed()
                            {
                                action
                                    .form_props
                                    .push(("NeumorphicDistance".into(), v.to_string()));
                            }
                        }
                        ui.end_row();
                        ...
                        ui.label("Rim tint");
                        {
                            let mut c = hex_to_color32(&form.neumorphic_params.rim_tint);
                            ui.horizontal(|ui| {
                                if color_edit_button_closing(ui, &mut c).changed() {
                                    action
                                        .form_props
                                        .push(("NeumorphicRimTint".into(), color32_to_hex(c)));
                                }
                            });
                        }
                        ui.end_row();
                        ...
                        ui.label("Rim weight");
                        {
                            let mut v = form.neumorphic_params.rim_weight;
                            if ui
                                .add(DragValue::new(&mut v).speed(0.1).range(0.0..=6.0))
                                .changed()
                            {
                                action
                                    .form_props
                                    .push(("NeumorphicRimWeight".into(), v.to_string()));
                            }
                        }
                        ui.end_row();
                        ...
                        ui.label("Rim blur");
                        {
                            let mut v = form.neumorphic_params.rim_blur;
                            if ui
                                .add(DragValue::new(&mut v).speed(0.1).range(0.0..=6.0))
                                .changed()
                            {
                                action
                                    .form_props
                                    .push(("NeumorphicRimBlur".into(), v.to_string()));
                            }
                        }
                        ui.end_row();
                    }
