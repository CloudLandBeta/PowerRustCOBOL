...
use crate::i18n::Tr;
use crate::project_model::UserControlDef;
...
const TOOLS: &[ToolEntry] = &[
    ...
    ToolEntry {
        label: "ListBox",
        ct: ControlType::ListBox,
        category: "Common",
    },
    ToolEntry {
        label: "NumericUpDown",
        ct: ControlType::NumericUpDown,
        category: "Common",
    },
    ToolEntry {
    ...
    ToolEntry {
        label: "GroupBox",
        ct: ControlType::GroupBox,
        category: "Container",
    },
    ToolEntry {
        label: "Panel",
        ct: ControlType::Panel,
        category: "Container",
    },
    ToolEntry {
        label: "TabControl",
        ct: ControlType::TabControl,
        category: "Container",
    },
    ToolEntry {
        label: "Splitter",
        ct: ControlType::Splitter,
        category: "Container",
    },
    // ── Data ───────────────────────────────────────────────────────────────────
    ToolEntry {
    ...
    ToolEntry {
        label: "TreeView",
        ct: ControlType::TreeView,
        category: "Data",
    },
    // ── Graphics ───────────────────────────────────────────────────────────────
    ToolEntry {
        label: "PictureBox",
        ct: ControlType::PictureBox,
        category: "Graphics",
    },
    ToolEntry {
        label: "Animator",
        ct: ControlType::Animator,
        category: "Graphics",
    },
    ToolEntry {
        label: "ProgressBar",
        ct: ControlType::ProgressBar,
        category: "Graphics",
    },
    ToolEntry {
        label: "Slider",
    ...
    ToolEntry {
        label: "StatusBar",
        ct: ControlType::StatusBar,
        category: "Menu",
    },
    // ── Non-visual ─────────────────────────────────────────────────────────────
    ToolEntry {
        label: "Timer",
        ct: ControlType::Timer,
        category: "NonVisual",
    },
    ToolEntry {
        label: "AgentObject",
        ct: ControlType::AgentObject,
        category: "NonVisual",
    },
    ToolEntry {
        label: "RestClient",
        ct: ControlType::RestClient,
        category: "NonVisual",
    },
    ToolEntry {
        label: "SqlDatabase",
    ...
    ToolEntry {
        label: "BarChart",
        ct: ControlType::BarChart,
        category: "Charts",
    },
    ToolEntry {
        label: "LineChart",
        ct: ControlType::LineChart,
        category: "Charts",
    },
    ToolEntry {
        label: "PieChart",
        ct: ControlType::PieChart,
        category: "Charts",
    },
    ToolEntry {
        label: "AreaChart",
        ct: ControlType::AreaChart,
        category: "Charts",
    },
    ToolEntry {
        label: "ScatterChart",
        ct: ControlType::ScatterChart,
        category: "Charts",
    },
    ToolEntry {
        label: "DonutChart",
                ...
                .show(ui, |ui| {
                    if filter_lo.is_empty() {
                        for &(cat_id, _) in CATEGORIES {
                            ...
                            let tools_in_cat: Vec<&ToolEntry> =
                                TOOLS.iter().filter(|e| e.category == cat_id).collect();
                            ...
                            if !collapsed {
                                ...
                                ui.painter().line_segment(
                                    ...
                                    Stroke::new(
                                        1.0,
                                        Color32::from_rgba_premultiplied(100, 120, 180, 60),
                                    ),
                                );
...
fn render_user_controls(
    ...
    let entries: Vec<&UserControlDef> = user_controls
        .iter()
        .filter(|def| filter_lo.is_empty() || def.name.to_ascii_lowercase().contains(filter_lo))
        .collect();
    if entries.is_empty() {
        return;
    }
    ...
    match ct {
        ...
        ControlType::Label => {
            painter.text(
                ...
                "A",
                egui::FontId::proportional(r * 2.2),
                color,
            );
        }
        ControlType::TextBox => {
            ...
            painter.line_segment(
                [Pos2::new(ix - r * 0.10, iy0), Pos2::new(ix + r * 0.10, iy0)],
                th,
            );
            painter.line_segment(
                [Pos2::new(ix - r * 0.10, iy1), Pos2::new(ix + r * 0.10, iy1)],
                th,
            );
        }
        ControlType::CheckBox => {
            ...
            painter.line_segment(
                [
                    ...
                    Pos2::new(b.min.x + bsz * 0.42, b.max.y - bsz * 0.18),
                ],
                s,
            );
            painter.line_segment(
                [
                    Pos2::new(b.min.x + bsz * 0.42, b.max.y - bsz * 0.18),
                    Pos2::new(b.max.x - bsz * 0.12, b.min.y + bsz * 0.18),
                ],
                s,
            );
            painter.line_segment(
                [
                    Pos2::new(c.x - r * 0.05, c.y),
                    Pos2::new(c.x + r * 0.95, c.y),
                ],
                th,
            );
        }
        ...
        ControlType::ComboBox => {
            ...
            painter.line_segment(
                [
                    Pos2::new(divx, b.min.y + 1.0),
                    Pos2::new(divx, b.max.y - 1.0),
                ],
                th,
            );
            ...
            painter.line_segment(
                [
                    Pos2::new(ac.x - r * 0.18, ac.y - r * 0.13),
                    Pos2::new(ac.x, ac.y + r * 0.13),
                ],
                s,
            );
            painter.line_segment(
        ...
        ControlType::NumericUpDown => {
            ...
            painter.line_segment(
                [
                    Pos2::new(ax - r * 0.15, midy - r * 0.07),
                    Pos2::new(ax, b.min.y + r * 0.20),
                ],
                th,
            );
            painter.line_segment(
        ...
        ControlType::DateTimePicker => {
            ...
            for bx in [b.min.x + b.width() * 0.28, b.min.x + b.width() * 0.72] {
                painter.line_segment(
                    [Pos2::new(bx, b.min.y - 1.0), Pos2::new(bx, b.min.y + 2.5)],
                    s,
                );
            }
            for row in 0..2 {
        ...
        ControlType::TabControl => {
            ...
            painter.line_segment(
                [
                    Pos2::new(tab.max.x, tab.min.y),
                    Pos2::new(tab.max.x, tab.max.y),
                ],
                s,
            );
            ...
            painter.line_segment(
                [
                    Pos2::new(t2min.x + r * 0.9, t2min.y),
                    Pos2::new(t2min.x + r * 0.9, body.min.y),
                ],
                th,
            );
        }
        ...
        ControlType::TreeView => {
            ...
            let children: &[(f32, f32)] = &[
                (c.x + r * 0.2, c.y - r * 0.3),
                (c.x + r * 0.2, c.y + r * 0.45),
            ];
            ...
            painter.line_segment(
                [
                    Pos2::new(children[0].0, children[0].1 + 1.8),
                    Pos2::new(children[0].0, gc.1),
                ],
                th,
            );
            painter.line_segment(
                [Pos2::new(children[0].0, gc.1), Pos2::new(gc.0 - 1.8, gc.1)],
                th,
            );
        ...
        ControlType::PictureBox => {
            ...
            painter.line_segment(
                [
                    Pos2::new(b.min.x, b.max.y),
                    Pos2::new(b.min.x + b.width() * 0.38, my),
                ],
                s,
            );
            painter.line_segment(
                [
                    Pos2::new(b.min.x + b.width() * 0.38, my),
                    Pos2::new(b.min.x + b.width() * 0.62, b.max.y),
                ],
                s,
            );
            ...
            painter.line_segment(
                [
                    Pos2::new(b.min.x + b.width() * 0.50, b.max.y),
                    Pos2::new(b.min.x + b.width() * 0.76, my2),
                ],
                s,
            );
            painter.line_segment(
        ...
        ControlType::Animator => {
            ...
            for i in 0..3 {
                ...
                painter.line_segment(
                    [
                        Pos2::new(sx, b.min.y),
                        Pos2::new(sx, b.min.y + b.height() * 0.16),
                    ],
                    th,
                );
                painter.line_segment(
                    [
                        Pos2::new(sx, b.max.y),
                        Pos2::new(sx, b.max.y - b.height() * 0.16),
                    ],
                    th,
                );
            }
        ...
        ControlType::ProgressBar => {
            ...
            for i in 0..3 {
                ...
                painter.line_segment(
                    [
                        Pos2::new(sx, fill.min.y),
                        Pos2::new(sx - fill.height() * 0.5, fill.max.y),
                    ],
                    th,
                );
            }
        ...
        ControlType::Timer => {
            ...
            painter.line_segment(
                [
                    Pos2::new(c.x, c.y - r * 0.82),
                    Pos2::new(c.x, c.y - r * 1.02),
                ],
                th,
            );
            painter.line_segment([c, Pos2::new(c.x + r * 0.36, c.y - r * 0.58)], s);
            painter.line_segment(
                [c, Pos2::new(c.x + r * 0.10, c.y - r * 0.42)],
                Stroke::new(1.8, color),
            );
            painter.circle_filled(c, 1.3, color);
            painter.line_segment(
                [
                    Pos2::new(c.x - r * 0.16, c.y - r * 1.02),
                    Pos2::new(c.x + r * 0.16, c.y - r * 1.02),
                ],
                s,
            );
        ...
        ControlType::AgentObject => {
            ...
            painter.line_segment(
                [Pos2::new(c.x, head.min.y), Pos2::new(c.x, c.y - r * 1.08)],
                th,
            );
        ...
        ControlType::BarChart => {
            ...
            painter.line_segment(
                [
                    Pos2::new(c.x - total_w * 0.55, base_y),
                    Pos2::new(c.x + total_w * 0.55, base_y),
                ],
                th,
            );
        ...
        ControlType::PieChart => {
            ...
            painter.circle_filled(
                Pos2::new(c.x + mid.cos() * mr, c.y + mid.sin() * mr),
                r * 0.3,
                dim,
            );
        }
