...
fn methods_for_type(ctrl_type: &str) -> Vec<Method> {
    let (base, specific): (&[Method], &[Method]) = match ctrl_type {
        ...
        "KnowledgeBase" => (
            UNIVERSAL_NONVISUAL,
            &[
                ("CreateCollection", "Create a collection (name) — 1/0"),
                ("RemoveCollection", "Move a collection aside (name) — 1/0"),
                ("ListCollections", "List the collections; answers how many"),
                ("GetCollection", "The n-th collection's name (after ListCollections)"),
                ("AddDocument", "Save a document (name, text) and index it — onIndexed"),
                ("UpdateDocument", "Replace a document (name, text) — onIndexed"),
                ("ImportDocument", "Copy a file in (path [, name]) and index it — onIndexed"),
                ("DeleteDocument", "Delete a document (name) — onIndexed"),
                ("ListDocuments", "List the collection's documents; answers how many"),
                ("GetDocument", "The n-th document's name (after ListDocuments)"),
                ("Refresh", "Index what changed in the folder — onIndexed"),
                ("Reindex", "Index everything again, with this control's embedder"),
                ("Search", "Search (query [, max]) — onSearchComplete"),
                ("GetResultDocument", "The n-th hit's document"),
                ("GetResultHeading", "The n-th hit's section"),
                ("GetResultPassage", "The n-th hit's text"),
                ("GetResultScore", "The n-th hit's score"),
                ("FetchModel", "Fetch the built-in model into the application's folder"),
