...
impl CoboltApp {
    ...
    fn do_run(&mut self) {
        ...
        if self.cobolt_project.is_some() {
            self.launch_project_main_form(false);
            return;
        }
    ...
    /// Run — or, with `debug`, debug — the open (desktop) project: resolve the
    /// MAIN form — repairing the exactly-one invariant if needed — and launch
    /// it as a standalone `rcrun run-form` process. An open designer's live
    /// state wins; a closed main form is loaded from disk (its `.cbl` was just
    /// regenerated by the caller's `regenerate_all_forms`). The project Run
    /// and Debug buttons share this so they can never point at different
    /// forms: Debug with no source open debugs what Run would run.
    fn launch_project_main_form(&mut self, debug: bool) {
        ...
        if forms.is_empty() {
            self.output.clear_run_output();
            self.output.push_status(format!(
                "A desktop project needs at least one form — create one to {verb}."
            ));
            return;
        }
        let holder = match crate::main_form::normalize_main_form(&dir, &forms) {
            ...
            Err(e) => {
                self.output.push_status(format!("{verb}: {e}"));
                None
            }
        };
        let Some(rel) = holder else {
            self.output
                .push_status(format!("{verb}: the project has no main form."));
            return;
        };
        let abs = dir.join(&rel);
        // Open in a designer → its Run Form / Debug Form path (saves dirty
        // state first).
        ...
        match load_form(&abs) {
            Ok(form) => self.spawn_form_run(abs, form, debug),
            Err(e) => {
                self.output
    ...
    /// The child environment for launching the BUILT application — for Run or
    /// for Debug.
    ///
    /// It names the form the developer actually pressed the button on. A built
    /// application starts at its main form and refuses any other, which is right
    /// for something you have distributed but wrong for the designer: in a
    /// project containing one `EXEC RUST` block anywhere, EVERY launch goes
    /// through the built binary, so without this both Run Form and Debug opened
    /// the main form instead of the one on screen.
    ///
    /// `main_form_guard::form_id` is the SAME function the build keys its
    /// embedded table with, called rather than re-derived: a second spelling of
    /// "uppercased stem" would fail silently, as a binary refusing a form it
    /// really does contain.
    ///
    ...
    fn run_built_binary(&mut self, binary: &Path, form_path: &Path) {
        ...
        let envs = self.built_app_env(form_path);
    ...
    fn do_debug(&mut self) {
        ...
        let Some((path, src)) = self.editor.active_source() else {
            // No source open: debug the project's MAIN form — the same target
            // Run launches — with the debugger attached. Debug used to return
            // here silently, and its button was greyed to match, so a project
            // just built with no tab open could be run but not debugged
            // (operator, 2026-09-19).
            if self.cobolt_project.is_some() {
                self.launch_project_main_form(true);
            }
            return;
        };
        let path = path.clone();
    ...
    fn show_stale_build_prompt(&mut self, ctx: &Context) {
        ...
        if run_anyway {
            ...
            self.stale_build_prompt = None;
            match prompt.intent {
                StaleBuildIntent::Run => self.launch_project_main_form(false),
...
impl eframe::App for CoboltApp {
    ...
    fn ui(&mut self, root_ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        // the editor — the file it debugs — OR a compilable project, whose MAIN
        // form it debugs when no source is open, exactly as Run launches it.
        //
    ...
    fn show_designer_window(&mut self, panel_ui: &mut egui::Ui, idx: usize, tr: &Tr) {
        ...
        egui::Panel::top(format!("dtb_{idx}"))
            ...
            .show(panel_ui, |ui| {
                ...
                //
                // The tuple is the pending launch's INTENT (Run or Debug); only
                // its path matters here.
                let run_busy = self
