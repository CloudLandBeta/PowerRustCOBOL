#![cfg(feature = "render")]
// The Gauge's needle exists and takes the developer's `Color` on every
// surface. Regression net for the 2026-08-15 report ("gauge not showing
// needle; preview shows the needle, but its color is not the one defined for
// it in RAD"): one control rendered through the three real paths — designer
// canvas (`render_faces`, Static), preview (`render_form` Interactive over
// designed props) and run form (`render_form` Interactive over the
...
/// Donut, Value 80, StrokeWidth 15, Color #16A34AFF, `ShowNeedle` left at its
/// default. It painted no needle on any surface.
#[test]
fn a_donut_gauge_paints_its_needle_in_the_defined_colour_on_every_surface() {
    ...
    assert_needle_everywhere(&g, Color32::from_rgb(0x16, 0xA3, 0x4A), "Donut");
}

/// Same guarantee for the Radial, which always had the needle — the colour
/// must be the developer's, not a theme's.
#[test]
