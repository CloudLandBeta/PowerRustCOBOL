...
fn numeddot_suite_reports_pass() {
    ...
    assert!(
        out.contains("RESULT       : PASS"),
        "numeddot suite failed:\n{out}"
    );
    assert!(
        !out.contains("FAIL T0"),
        "numeddot reported failures:\n{out}"
    );
    assert_eq!(
        out.matches("PASS T0").count(),
        43,
        "expected 43 PASS lines:\n{out}"
    );
}
...
fn numedcom_suite_reports_pass() {
    ...
    assert!(
        out.contains("RESULT       : PASS"),
        "numedcom suite failed:\n{out}"
    );
    assert!(
        !out.contains("FAIL T0"),
        "numedcom reported failures:\n{out}"
    );
    assert_eq!(
        out.matches("PASS T0").count(),
        41,
        "expected 41 PASS lines:\n{out}"
    );
}
...
fn zero_suppression_and_comma() {
    assert_eq!(
        edit(
            "ZZZ,ZZ9.99",
            "       01 S PIC 9(6)V99 VALUE 1234.50.",
            "           MOVE S TO E"
        ),
        "[  1,234.50]"
    );
