...
fn indexed_legacy_string_list() {
    ...
    assert!(out.contains("i1=[beta"), "Items(1) line index failed: {out}");
...
fn collection_element_delete() {
    ...
    assert!(out.contains("row0=[b"), "delete did not shift elements: {out}");
}
...
fn property_receiver_for_all_verbs() {
    ...
    assert!(out.contains("str=[ABCD"), "STRING INTO property failed: {out}");
...
fn initialize_member_and_control_rules() {
    ...
    assert!(out.contains("val=[]"), "INITIALIZE OBJ should clear Value: {out}");
    assert!(out.contains("cap=[txt"), "INITIALIZE OBJ must not touch Caption: {out}");
    // INITIALIZE OBJ::Value WS-N: Value cleared; WS-N reset to SPACES per PIC.
    assert!(out.contains("val2=[]"), "INITIALIZE OBJ::Value should clear it: {out}");
...
fn method_statement_has_no_effect_on_data() {
    ...
    assert!(out.contains("v=[hello"), "method statement must not mutate the value: {out}");
}
