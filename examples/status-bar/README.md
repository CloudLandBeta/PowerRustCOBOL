# StatusBar — control test example

Subject control: **StatusBar**. The form fires a console `DISPLAY "<Event> working"`
...
```sh
rcrun build examples/status-bar/cobolt.toml
```
...
> `generated/status-bar.cbl` is codegen output (regenerable from the form);
